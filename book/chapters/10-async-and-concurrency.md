...
> **Implementation status.** The `scope` / `spawn` syntax compiles today, and the guarantees above are landing in slices (decision-log D119). The structural "a spawned task must be consumed — awaited or scope-joined" check (diagnostic `V0014`, slice S2) is **shipped** and enforces guarantee 1 (no leaks / no fire-and-forget) on all backends. Real *forked* concurrency is landing per backend: JVM via a virtual-thread `ExecutorService` (slice S4, D120 — guarantees 1 and 3, first-failure propagation; sibling-cancel and aggregation of guarantees 2 and 4 are a JDK-25 `StructuredTaskScope` follow-up), and MSIL via `Task.WhenAll` + a linked `CancellationTokenSource` (slice S6). Until a backend's runtime slice lands, `spawn` exposes the underlying task and `scope` is a lexical block, so the guarantees hold only degenerately (every spawned call completes at its spawn site). Native async is genuinely concurrent (D-N-022), but its structured-guarantee layer — sibling-cancel and aggregation — is a separate follow-up.
