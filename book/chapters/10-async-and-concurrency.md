...
> **Implementation status.** The `scope` / `spawn` syntax compiles today, and the guarantees above are landing in slices (decision-log D119). The structural "a spawned task must be consumed — awaited or scope-joined" check (diagnostic `V0014`, slice S2) is **shipped** and enforces guarantee 1 (no leaks / no fire-and-forget) on all backends. Real *forked* concurrency is landing per backend: on **JVM it is shipped** (slice S4, D120 / D-progress-602) — `scope { }` opens a virtual-thread `ExecutorService` closed on every exit path, `spawn e` submits a synthesized `Callable` yielding a `Future`, and `await` joins it (guarantees 1 and 3, first-failure propagation; sibling-cancel and aggregation of guarantees 2 and 4 are a JDK-25 `StructuredTaskScope` follow-up). MSIL is still degenerate pending `Task.WhenAll` + a linked `CancellationTokenSource` (slice S6). Until a backend's runtime slice lands, `spawn` exposes the underlying task and `scope` is a lexical block, so the guarantees hold only degenerately (every spawned call completes at its spawn site). Native async is genuinely concurrent (D-N-022), but its structured-guarantee layer — sibling-cancel and aggregation — is a separate follow-up.
...
::: sidebar
**On `--target native`.** Everything above works on the LLVM native target too, with one machine substituted for another: an `async func` compiles to an LLVM coroutine driven by a cooperative single-threaded scheduler in the native runtime (not a .NET `Task`), `spawn` binds a hot task that runs until its first suspension, and spawned tasks genuinely interleave — `Std.Time.sleepMillis` inside an async body suspends only the calling task, and `Std.Process.runCapture` / `runCaptureWithInput` capture their subprocess without blocking the others (stdin content and the `timeoutMs` deadline kill both honored, in async and plain contexts alike; the kill takes the child's whole process group, so a timed-out shell pipeline doesn't leave grandchildren running). Cancellation (§10.2) is the exception: native has no cancellation tokens, and a failing task aborts the process (panics abort on native). Async generators (§10.5) are rejected at compile time on native.
:::
...
- `func` operations are exclusive as well, so a `func` never sees an entry half-way through. (Barriers are implemented on `--target dotnet` and `--target jvm`; `--target native` rejects a `when:` clause at build time for now.)
