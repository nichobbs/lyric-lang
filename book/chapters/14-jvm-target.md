...
```sh
java --module-path target/build/module-path.txt:target/build/account.jar \
     --module lyric.account/lyric.account.Account
```

`lyric build` writes `target/build/module-path.txt` listing all dependency JARs in order, including the Lyric stdlib JARs. The run-script wrapper generated by `lyric run --target jvm` invokes this automatically.
...
Lyric `Char` is a Unicode scalar value (not a UTF-16 code unit). It maps to `int` on the JVM. There is no implicit conversion to Java `char`. Use `String.codePointAt` / `String.codePointCount` to go between Lyric `Char` values and Java `String`. A conversion helper `Char.toUtf16Pair(c): (Char, Char?)` is provided in `lyric.std.text.unicode` for code that genuinely needs surrogate pairs.
...
public final record Account(long id, long balance)
    implements lyric.runtime.LyricRecord { … }
```

The `LyricRecord` marker interface lets debuggers and LSP plugins distinguish Lyric records from arbitrary Java records.

An opaque type compiles to a final class with mangled private fields (names prefixed with `$`) and no public accessors. Construction is package-private; cross-module callers use the `pub` factory functions the Lyric source exposes. The class is annotated `@LyricOpaque` for tooling.
