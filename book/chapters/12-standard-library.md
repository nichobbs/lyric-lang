...
A persistent `Map` (and, as a stretch goal, a `Set`) were also planned for this module. Building them surfaced reproducible self-hosted MSIL backend bugs with no working precedent anywhere in this codebase — a classic cons-list/BST node shape (a case field typed as the enclosing generic type's own open instantiation) corrupts field reads at runtime, and a generic function that invokes a closure parameter inside a loop over a `slice[UserGenericRecord[K, V]]` parameter (the natural shape for a comparator/equality-keyed map) monomorphizes with a corrupted first-parameter signature. Both are deferred to a tracked follow-up rather than shipped broken or stubbed; see the module's header doc comment (`lyric-stdlib/std/collections_persistent.l`) for the full detail.
