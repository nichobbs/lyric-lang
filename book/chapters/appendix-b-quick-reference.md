...
| `T0126` | A `for` loop iterates over an expression whose type isn't recognized as iterable: not `slice[T]`, `List[T]`, a `Map[K, V]` key/value collection, a range, or a single-type-parameter `extern type` (the phantom-type-param idiom for a foreign collection). Iterating a Lyric-native single-param generic that merely has one type parameter but isn't a collection — `Option[T]` is the concrete case — used to silently type the loop element and fail at runtime instead (#6720). |
