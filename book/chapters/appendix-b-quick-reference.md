...
| `Std.HttpServer` | Low-level HTTP(S) server (`lyric-web` builds on this); on `--target dotnet` a pure-Lyric sans-IO engine over `System.Net.Sockets`/`SslStream` (the `HttpListener` server was retired, docs/61 §6) | `startListener`, `startListenerTls` (real TLS on both `--target dotnet` and `--target jvm`; dotnet returns `InvalidConfig` for a `requireClientCert`-without-`clientCa` mTLS misconfig, docs/61 §6.3), `nextContext`, `respondText`/`respondJson`/`respondBytesWithHeaders` |
