...
///
/// Compiles `compiler/lyric/lyric/typechecker_self_test.l` via the
...
let private findSelfTestSource () : string option =
    ...
    while found.IsNone && dir.IsSome do
        let candidate =
            Path.Combine(dir.Value.FullName, "lyric", "lyric", "typechecker_self_test.l")
    ...
    testList "Lyric.TypeChecker self-host (Phase 5 §M5.1)" [
        ...
        testCase "[typechecker_self_test_passes]" <| fun () ->
            let src =
                ...
                | None ->
                    failwith
                        "cannot locate compiler/lyric/lyric/typechecker_self_test.l — run from the source tree"
