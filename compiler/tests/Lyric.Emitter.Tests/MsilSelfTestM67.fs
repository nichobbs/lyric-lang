/// Stage M67 checked unsigned conversions — conv.ovf.u1 (0xB4), conv.ovf.u2 (0xB6), conv.ovf.u4 (0xB8), conv.ovf.u8 (0xBA).
module Lyric.Emitter.Tests.MsilSelfTestM67
...
let tests =
    testList "Msil.SelfTest M67 (conv.ovf.u1/u2/u4/u8)" [

        testCase "msil_self_test_m67" <| fun () ->
            let src =
                match findMsilSource "msil_self_test_m67.l" with
                | Some path -> File.ReadAllText path
                | None      -> failwith "cannot locate compiler/lyric/msil/msil_self_test_m67.l"
            ...
            let result, stdout, stderr, exitCode = compileAndRun "msil_self_test_m67" src
            ...
            for check in [ "dos_ok"; "pe_sig_ok"; "clr_cb_ok"; "entry_tok_ok"
                           "tiny_hdr_ok"; "conv_ovf_u1_ok"; "conv_ovf_u2_ok"
                           "conv_ovf_u4_ok"; "conv_ovf_u8_ok"; "bsjb_ok" ] do
