...
let tests =
    testList "Msil.SelfTest M72 (bge/ble/blt)" [

        testCase "msil_self_test_m72" <| fun () ->
            let src =
                match findMsilSource "msil_self_test_m72.l" with
                | Some path -> File.ReadAllText path
                | None      -> failwith "cannot locate compiler/lyric/msil/msil_self_test_m72.l"

            let dllPath = "/tmp/lyric_msil_m72_bge_ble_blt.dll"
            let cfgPath = "/tmp/lyric_msil_m72_bge_ble_blt.runtimeconfig.json"

            writeMsilRuntimeConfig cfgPath
            if File.Exists dllPath then File.Delete dllPath

            let result, stdout, stderr, exitCode = compileAndRun "msil_self_test_m72" src
            ...
            for check in [ "dos_ok"; "pe_sig_ok"; "clr_cb_ok"; "entry_tok_ok"
                           "tiny_hdr_ok"; "bge_ok"; "ble_ok"; "blt_ok"; "bsjb_ok" ] do
