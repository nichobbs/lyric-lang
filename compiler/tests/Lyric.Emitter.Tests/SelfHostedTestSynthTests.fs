...
///
/// Compiles `compiler/lyric/lyric/test_synth_self_test.l` via the
...
/// library from `compiler/lyric/lyric/test_synth/*.l` transparently.
///
    ...
    testList "Lyric.TestSynth self-host (Phase 5 §M5.x test runner port)" [
        ...
        testCase "[test_synth_self_test_passes]" <| fun () ->
            let src =
                ...
                | None ->
                    failwith
                        "cannot locate compiler/lyric/lyric/test_synth_self_test.l — run from the source tree"
