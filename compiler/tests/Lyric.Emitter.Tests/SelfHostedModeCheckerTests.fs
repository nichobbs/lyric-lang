...
///
/// Compiles `compiler/lyric/lyric/modechecker_self_test.l` via the
...
let tests =
    testList "Lyric.ModeChecker self-host (Phase 5 §M5.2)" [

        testCase "[modechecker_self_test_passes]" <| fun () ->
            let src =
                match findSelfTestSource () with
                | Some path -> File.ReadAllText path
                | None ->
                    failwith
                        "cannot locate compiler/lyric/lyric/modechecker_self_test.l — run from the source tree"
