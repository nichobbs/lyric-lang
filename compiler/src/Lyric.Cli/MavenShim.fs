...
///
/// Bootstrap-grade scope (intentional cuts):
///   - Static methods only; instance methods and constructors deferred.
///   - Java primitives + `java.lang.String` + types from the same
///     package only; generic, array, or unknown types produce skip
///     entries.
///   - No checked-exception wrapping in the shim (`Result[T, JvmException]`
///     is added by the emitter during call-site lowering, per §5 of
///     the spec).
    ...
    | Some classes ->
        ...
        for cls in sortedClasses do
            ...
            for m in staticMethods do
                ...
                | Ok retLyric, None ->
                    let baseName = lyricTypeName + "_" + m.Name
                    let key = (baseName, List.length m.Params)
                    if not (globalSeen.Add key) then
                        recordSkip qual
                            (sprintf "duplicate (%s, arity=%d) — overload \
                                      kept earlier" baseName (List.length m.Params))
                    else
                        let argList =
                            paramMaps
                            |> List.map (fun (pname, r) ->
                                ...
                                | Error _ -> "")
                            |> String.concat ", "
                        let methodName, renamed = safeIdent baseName
                        if renamed then
                            sb.AppendLine
            ...
            if not (List.isEmpty staticMethods) then
                sb.AppendLine "" |> ignore
