...
//
// `JvmByteBuilder` and `JvmConstantPool` are mutable BCL objects held
// by reference inside Lyric opaque types.  Because Lyric record fields are
// readonly, the Lyric side never replaces the reference — it only calls
// methods on the object through `@externTarget`-routed static statics in
// `JvmByteHost` / `JvmPoolHost`.  All state mutation stays in-place here.
