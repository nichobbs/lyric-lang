...
///
/// Shared by `collectAwaitInners` and `collectYieldInners` — the
/// `collect` callback decides which node kinds are accumulated.
let private collectMatchingInners
        (collect: ExprKind -> Expr option)
        (fn: FunctionDecl)
        : Expr list =
    let acc = ResizeArray<Expr>()
    let rec walkExpr (e: Expr) : unit =
        match collect e.Kind with
        | Some inner -> acc.Add inner; walkExpr inner
        | None ->
        match e.Kind with
        ...
        | EInterpolated segs ->
            for s in segs do
                ...
                | ISText _ -> ()
                | ISExpr e -> walkExpr e
        | EParen e | EAwait e | EYield e | ESpawn e | EOld e | EPropagate e | ETry e -> walkExpr e
...
let collectAwaitInners (fn: FunctionDecl) : Expr list =
    collectMatchingInners
        (function EAwait inner -> Some inner | _ -> None)
        fn
