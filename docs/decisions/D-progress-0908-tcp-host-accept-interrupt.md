# D-progress-908 — Native `Std.TcpHost`: portable accept() interrupt via a self-pipe + `poll(2)`, replacing the Linux-only `shutdown()`-on-listening-socket trick (#6806, closing the gap #6804 disclosed at N9.3 ship time)
