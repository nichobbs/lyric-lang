# D-progress-930 — #6581 follow-up: a genuine `auto_ffi_self_test.l` regression (over-broad `STNamedGenericInst` scoring) plus two review-flagged `emitGenericMethodExternCall` gaps (#6987, #6989)

**Status:** shipped

**Context.** After D-progress-929 landed as PR #6981, `claude-review` posted
...
D-progress-929's own `scoreSigType` change was discovered and fixed first.
...
VAR/MVAR anywhere in it — yet D-progress-929's arm scored ANY argument
...
`resolveExtern`'s two-pass split (D-progress-929's own comment on
...
its own top-of-function `STMVar` arm (D-progress-929, this PR's Gap 2 base)
...
**Related:** #6581 (D-progress-929, this entry's base), #6987 (fixed),
