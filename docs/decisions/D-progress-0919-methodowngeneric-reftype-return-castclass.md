# D-progress-919 — `emitGenericMethodExternCall`'s method-own-generic return narrows a genuine reference-type wrapper return, not just the value-type case (review follow-up)

**Status:** shipped

**Context.** A `claude-review` pass on PR #6981 flagged (SUGGESTION) that
`emitGenericMethodExternCall`'s return-handling code (D-progress-912, Gap 2)
...
**Reachability.** Unlike D-progress-915's `MValueTypeGenericInst` parameter
...
**Related:** #6581/D-progress-912 (Gap 2, this entry's base), D-progress-913
(#6989, the value-type side of this same return-narrowing dance),
D-progress-915 (the contrasting *unreachable* parameter-side case, for
comparison), PR #6981.
