# D-progress-892 — Port: native codegen destructor/closure/trampoline/vtable function-pointer bitcasts over-wrapped in an extra `NPtr` (#7030)
...
expected 'void (i8*)**'`. This PR's diff (JVM raw-erasure fix for
same-file generic union/record case fields, `jvm/codegen/*`) does not
touch the native/LLVM backend at all, so the failure is base-branch-wide,
not caused here.
