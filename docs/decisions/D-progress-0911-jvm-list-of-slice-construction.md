# D-progress-911 — JVM: `List[slice[T]]` (a `List` whose elements are arrays) is now constructible and indexable, closing two construction bugs plus a read-back gap the fix exposed (#6546)
