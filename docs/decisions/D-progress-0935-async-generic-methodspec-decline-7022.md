# D-progress-935 — `emitGenericMethodExternCall`'s missing `decl.isAsync` handling declined loudly, not silently, after the silent fallback proved unsafe (#7022)

**Status:** shipped

**Context.** `claude-review`'s next pass on PR #6981 (#6581) flagged that
`emitExternTargetBody`'s `msig.isGeneric` branch (Gap 2, D-progress-929)
...
But THIS PR added the `STMVar` arm (D-progress-929, Gap 2's own base) — so
...
**Related:** #6581/D-progress-929 (Gap 2, this entry's base), #7022
