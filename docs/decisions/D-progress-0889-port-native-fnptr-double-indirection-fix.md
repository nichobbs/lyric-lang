# D-progress-889 — Port: native codegen destructor/closure/trampoline/vtable function-pointer bitcasts over-wrapped in an extra `NPtr` (#7030)
...
expected 'void (i8*)**'`. This PR's diff does not touch the native/LLVM
backend at all, so the failure is base-branch-wide, not caused here.
...
D-progress-886/887/888/891's earlier BMod-port disclosures on these
sibling branches).
