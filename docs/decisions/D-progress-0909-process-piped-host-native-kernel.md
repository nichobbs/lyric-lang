# D-progress-909 — Native `Std.ProcessPipedHost`: real fork/pipe long-lived piped-child-stdio kernel over a new `lyric-rt` seam (#6142); `Std.Process`'s shared facade found genuinely unreachable on native by two independent, newly-filed compiler gaps
