# D-progress-913 — JVM codegen: the opaque-field mutator routing from D-progress-912 extended to the out/inout holder-argument codegen path (#5937 review follow-up, #6944)

**Status:** shipped

**Context.** Automated review of D-progress-912's PR (#6912→#6911) correctly
...
D-progress-912's fix did not touch.
...
exactly the two problems D-progress-912 fixed for plain assignment
...
D-progress-912 introduced in `05_stmts.l` (visible from `04_calls.l` — both
