# D-progress-904 — `Lyric.Weaver`'s `args.<field>` rewrite now descends into range expression bounds (and `forall`/`exists` binders); `Lyric.Doc` catches up to three `Lyric.Fmt` AST-fidelity fixes (annotation positioning, range bounds) (#6833, #6832)
...
rewrite mechanism this closes a gap in). See also D-progress-905, the
sibling parser fix from the same session (#6861; renumbered from its
