# D-progress-916 — MSIL codegen: a qualified generic type's own HEAD now resolves to the exact package named, closing the #6992 gap left by D-progress-915's `TRef`-only fix

**Status:** shipped

**Context.** A `claude-review` REQUIRED finding on PR #6904 (D-progress-915)
...
head, headSeg)` — the same helper D-progress-915 introduced — passing the
...
mirroring the D-progress-917 test-harness dead end for a different reason.
...
**Related:** #6992 (this fix), D-progress-915/#6904 (the `TRef`-only
