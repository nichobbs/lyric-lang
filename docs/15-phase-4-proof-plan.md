...
| `V0031` | Proof-required function carries an aspect annotation (`@MyAspect`); proofs verify the unelaborated body — contracts on the aspect's `around { }` block are not discharged | warning | mark function `@runtime_checked` or hand-inline the aspect for proof |
