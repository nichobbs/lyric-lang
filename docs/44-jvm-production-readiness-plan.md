...
| m-95 | A `Self`-typed interface/impl/record-method RETURN (`func dup(self: in Self): Self`) always erased to `java/lang/Object` at every call site — the JVM analog of the non-receiver `Self`-PARAM gap #6421/#6426 fixed, but for the RETURN position, which #6426's own investigation explicitly scoped out (D-progress-755) and #6431 tracked as a follow-up. A member access through the call result (`b.dup().n`) hit the erased-receiver refusal (J007). **Fixed (D-progress-756):** `narrowSelfCallResult` (`codegen/04_calls.l`) emits a `checkcast` to the receiver's own known concrete class right after a non-interface `invokevirtual` dispatch, narrowing the tracked call-result type from `Object` to that class, whenever the callee's registered sig carries a new `retIsSelf: Bool` field (set by `registerInstanceSigErased`/`06_items.l` for a bare-`Self`-declared return). Sound because `checkImplConformance` guarantees a `Self`-declared return yields exactly the receiver's own concrete type. An interface-typed receiver stays correctly erased and unreachable in practice: the type checker already types such a call's result as the INTERFACE ITSELF (T0113) before codegen ever sees it. Investigating this fix found MSIL's PRE-EXISTING return-position handling is NOT a genuine call-site substitution mechanism — it silently relies on the same package-wide field-NAME-only `fieldTokensByName` fallback D-progress-753 fixed for the param case, throwing `InvalidCastException` when two same-package records share a field name, and a chained call (`b.dup().dup()`) fails even without a collision (MSIL's `methodRetTypes` erases the tracked return type to `MObject` unconditionally, so the second call can't even find a dispatch target) — both real, previously-undocumented, out-of-scope-for-this-fix MSIL gaps. Verified by `self_return_call_site_jvm_self_test.l` (6 cases, JVM-only — kept out of the dual-target `bare_func_ref_self_test.l` specifically because of the MSIL gaps above). The first draft wired the narrowing into only ONE of the three reachable instance-call dispatch paths (review finding #6437); the holder-param path (`lowerVirtualCallWithHolders` — reached by any callee with an `out`/`inout` param, checkcast placed after the invoke while the result is top-of-stack, BEFORE the holder write-backs) and the bare intra-impl sibling-call path (`lowerGeneralStaticCall`'s `ctx.selfClass` branch, #1722 — narrows against the enclosing method's own class) are now covered too, each pinned by its own test case. | `codegen/{01_types,04_calls,06_items}.l`, `bridge.l` | #6431 |
