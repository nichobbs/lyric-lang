...
| B-2 | ~~`?` lowers to a synchronous pass-through (no Err/None short-circuit)~~ **Resolved (D119 audit, 2026-07-05):** `?` is desugared to `match`/`return` by the shared middle-end pass `Lyric.Propagate.lowerPropagateFile`, run in `jvm/bridge.l` (and the MSIL/native bridges) before codegen; the codegen `EPropagate` pass-through is unreachable fallback. **Still open:** `async`/`await`/`spawn` remain synchronous on JVM (no future/virtual-thread lowering on the user path) — the real structured-concurrency + cancellation work is D119 slices S4/S7. Generators are **not** a sync pass-through: they lower to a lazy virtual-thread + `SynchronousQueue` producer (see M-12, corrected). | `codegen/02_exprs.l` (EAwait/ESpawn), `bridge.l` (`lowerPropagateFile`) | #2469 (generators), D119 (async/spawn) |
...
| M-18 | JVM distribution: ~~no `lyric run --target jvm` (#674)~~ **DONE** (`cli_run.l` builds the bundled JAR via the in-process `Jvm.Bridge` and execs `java -jar`); ~~`main(slice[String])` argv `VerifyError` + `Int` return not propagated to the exit code (#3303)~~ **Fixed (D-progress-543):** `isMainFunc` accepts the `argv: slice[String]` form, the synthesised `main(String[])` wrapper forwards the incoming `String[]` to the erased `Object[]` slice parameter (JVM array covariance, no copy), `args.count` lowers to `arraylength`, and an `Int`-returning main routes through `java.lang.System.exit(int)` (branchless — no StackMapTable needed); verified by `entry_args_jvm_main.l` + the CI argv/exit-code check; ~~`lyric bench --target jvm` (#680) blocked on JVM `Std.Time` (#3302)~~ **unblocked (D-progress-543):** `_kernel_jvm/time_host.l` is now pure Lyric over the JVM auto-FFI (no static-field externs, no phantom `lyric.stdlib.jvm.*` shim classes; `Duration.ZERO` → `ofMillis(0)`, arg-order/unit divergences bridged in Lyric bodies, fractional durations preserved via `ofNanos` + `Math.round`), verified by `time_jvm_self_test.l`; ~~GraalVM native-image path (#675/#1975)~~ **DONE (D131, D-progress-630):** `lyric build --release --aot --target jvm` stages the bundled JAR into `.lyric-release/` and drives GraalVM `native-image` over it (entry point from the JAR manifest's `Main-Class`; restored `[maven]` jars as the image classpath; `--no-fallback` unconditional so a JVM-requiring fallback image is a build failure rather than a silent downgrade; `--rid` validated against the host since native-image cannot cross-compile), Linux + macOS, Windows still open | #675, #1975 | #674 ✅; #680 ✅ (#3302 fixed); #3303 ✅; #675 ✅; #1975 ✅ (Windows open) |

| M-20 | ~~`lyric build --manifest ... --target jvm` on a project whose `[project.packages]` form a legitimate import cycle (e.g. `Web` ↔ `Web.Kernel.Runtime`) fails the ENTRY package's own qualified types with `error[T0014] unknown qualified type 'Web.Request'`~~ **DONE (#6024).** Root cause: `Jvm.Bridge.compileProjectToJarBundledWithFeatures` built its flat cross-package `importedPkgs` list from every stdlib file plus every NON-entry sibling project package, but never added the entry package's (`pkgSrcs[0]`) own items — so a sibling importing the entry package back saw its types as entirely unknown, even though a non-cyclic multi-package library (whose siblings never reference the entry package back) built fine, and the single-file bundled JVM path (which folds the whole transitive import closure into one flat registry, entry package included) resolved the identical cycle correctly. Fixed by seeding the entry package's own cross-package items into the same flat `importedPkgs` list every other package already joins. | `jvm/bridge.l:compileProjectToJarBundledWithFeatures` | #6024 |
| M-21 | ~~JVM target: `[dependencies]` path/workspace deps are never compiled into the bundle — only `pub aspect`/`pub wire` templates are~~ **Core mechanism DONE as a side effect of #6136 (D-progress-720; predates this fix).** `resolveManifestDependencies` collects a `path`/`workspace` dependency's package sources into `depTemplateSrcs` unconditionally, and `buildProjectFromManifest` / `emitSingleFileOrProject` append those collected packages to the bundle for `target = Jvm`. **Residual fixed here (#6264):** `resolveManifestDependencies`'s `Path` branch still treated an unbuilt `.dll` as FATAL (`hadUnbuiltLocalDep`) regardless of target — a Dotnet-only concern, since the JVM bundler never reads `restoredDllPaths` — which blocked `emitSingleFileOrProject`'s single-file build path (the only caller gating on it) for a `--target jvm` build of a fresh checkout whose path dependency had never been built for `--target dotnet` first. A dependency source-file read failure (previously silently non-fatal on every target) is now also fatal specifically on Jvm, matching the project's own "sibling packages are FATAL" policy. | `cli/workspace_builder.l:resolveManifestDependencies` | #6264 |
| M-22 | ~~JVM target: no compiled-bundle fallback for stdlib type resolution when raw `lyric-stdlib/std/` source is unreachable — `Std.Core`'s types never resolve for an installed-SDK-only JVM build~~ **DONE (#4994, the JVM counterpart of #4980/M-10's `.NET` fix).** Ships a JAR-resident equivalent of `Lyric.ContractMeta.readAllContractsFromFile`: `readAllContractsFromJarFile` reads `Lyric.Contract.<Pkg>` entries out of a JAR's ZIP central directory via the existing pure-Lyric `Jvm.ZipReader` (built for JDK `.jmod` / Maven-JAR auto-FFI resolution — no new ZIP-parsing code), and `Lyric.RestoredPackages.loadRestoredPackageJvm` + `Emitter.stdlibSourcesFromCompiledBundleJvm` reconstruct declaration-only stdlib source per package, mirroring the `.NET`-side `stdlibSourcesFromCompiledBundle`. The WRITER half — `Jvm.Bridge` never embedded any contract metadata into a JAR at all before this — is new: `compileProjectToJarBundledWithFeatures` now embeds one `Lyric.Contract.<Pkg>` ZIP entry per PROJECT package (parity with the MSIL bridge's `embedLyricContract` scope), which is what will make a real `lyric build --manifest lyric-stdlib/lyric.toml --target jvm` produce a self-describing `Lyric.Stdlib.Jvm.jar` going forward. The writer also mirrors `Msil.Bridge`'s `withGenericFuncsForContract`: `Lyric.Mono` strips a same-file generic `pub func` once its local call sites are specialised, which would otherwise silently drop declarations like `Std.Core.unwrapOr`/`Std.Collections.mapKeys` from the embedded contract (review finding on PR #6558, fixed before merge). | `jvm/bridge.l` (`contractZipEntry`, `withGenericFuncsForContract`, writer), `contract_meta.l` (`readAllContractsFromJarFile`), `restored_packages.l` (`loadRestoredPackageJvm`), `emitter.l` (`stdlibSourcesFromCompiledBundleJvm`, `findCompiledStdlibDirJvm`) | #4994 |
| M-23 | ~~JVM target: `for`-loop over an extern generic collection (the `MapKeyCollection`/`MapValueCollection` phantom-type-param idiom applied to a THIRD-PARTY extern type, e.g. `Web.Kernel.Runtime`'s `JHttpStringCollection[T]`) erases the loop element's static type to `Object`; separately, `lowerMethodCall`'s no-metadata fallback blanket-guessed `()Object` for `toString()`/`hashCode()`/`equals()` too, so a chained Lyric stdlib method off `.toString()`'s untyped result failed auto-FFI resolution~~ **DONE (D-progress-818).** `SFor`'s element-type resolution now falls back to ANY single-type-parameter generic's sole type argument (not just the literal name `List`), and the metadata-unresolvable `JRef(cls)` fallback in `lowerMethodCall` pins the real, JVM-spec-fixed signatures for `toString()`/`hashCode()`/`equals(Object)` instead of guessing `()Object` for those three specifically — every other unknown method on an unresolvable receiver still degrades to `()Object` unchanged. Directly extends M-15's `java.lang.Object`-superclass-walk fix to the receiver-metadata-unresolvable case M-15 didn't cover. Unblocks `lyric build --manifest lyric-web/lyric.toml --target jvm`. **Follow-up (D-progress-819, #6718):** the pinned `equals(Object)` descriptor left a raw JVM primitive unboxed on the stack for a primitive-typed argument (e.g. `x.equals(5)`), a stack-type/descriptor mismatch the verifier rejects as `VerifyError` at class-load time — fixed by boxing via the existing `coerceArgTo` primitive-to-Object path before the pinned `invokevirtual`. | `type_checker/typechecker_stmts.l` (`SFor`), `jvm/codegen/04_calls.l` (`lowerMethodCall`) | #6565, #6718 |
...
| m-6 | ~~JVM regex daemon-thread timeout shim~~ **Fixed.** `java.util.regex.Pattern` has no timeout of its own; `Std.RegexHost.Regex` on JVM is now a record pairing the compiled `Pattern` with a `timeoutMs`, and `hostIsMatch`/`hostMatchOne`/`hostReplace` each race their work on a fresh daemon `Thread` (`Thread.join(timeoutMs)` — a genuinely bounded wait, unlike `Future.get()`), using the JVM backend's existing Lyric-closure-to-`Runnable` SAM bridging. Verified against a real catastrophic-backtracking pattern (`((a+)+)+$`) hitting the compiled-in deadline and returning `Err(TimedOut)` (`lyric-compiler/jvm/regex_redos_jvm_main.l`). | `_kernel_jvm/regex_host.l` | #1103 ✅ |
...
| m-13 | ~~`lowerTryCatchExpr` value-less catch arm (type-checker gap #2042) aborted with a bare `panic(...)` carrying no source location~~ **DONE (D-progress-509).** Now a source-located `error[J004]: <line>:<col>: …` built from the offending catch clause's span, re-emitted under `error[J002]` by the `Jvm.Bridge` `Bug` catch for bundled packages. `J004` is the next free code after `J001`–`J003`. Regression-guarded by `try_catch_expr_jvm_self_test.l` (4 valid cases) in CI on `--target jvm`. **#3310 follow-up (fixed):** the `line:col` prefix alone named no package, unhelpful once a build bundles dozens of files; the message is now `error[J004]: <dotted pkgName>:<line>:<col>: …`, splicing in `ctx.pkgName` (dotted) right after the code — the same package identifier `Jvm.Bridge`'s `error[J002]` wrapper already names for a referenced-package codegen failure — while keeping `error[J004]` at the very front so the `b.message.startsWith("error[J0")` passthrough guard still recognises it. Regression-guarded by `jvm_trycatch_bridge_self_test.l` (drives `Jvm.Bridge.compileToJarBundled` in-process and asserts on the caught `Bug` message). | `codegen/05_stmts.l` | #3193, #3310 |
...
| m-20 | ~~An extern type used in another package's signatures/locals (`Std.Time.now(): Instant` with `Instant` declared in `Std.TimeHost`) resolved to the nonexistent `<pkg>/<Alias>` class → `NoClassDefFoundError` at link~~ **Fixed (D-progress-543):** the bundled compile builds a per-package extern-type map and seeds each file's signature registration + codegen with the union of its imports' extern types (`externSeedForFile` → `collectFileSigsSeeded`/`codegenPackageWithSigsSeeded`, own declarations win); local `val`/`var` annotations, `is`-type-tests, and result-wrap returns now resolve through `typeExprToJvmExtern` with the seeded map. **Residual gap fixed (#5976):** the original seed only unioned a package's extern-type map into a file's `externTypes` when that file DIRECTLY imported the declaring package — an extern type reachable only TRANSITIVELY (`Std.RegexSafe.tryCompile(): Result[Std.RegexHost.Regex, RegexError]`, where `Regex` is declared in `Std.RegexHost` and a consumer imports `Std.RegexSafe` but never `Std.RegexHost`) still fell to the "user type in my own package" guess at a `retGenericArgs` match-bind site (`NoClassDefFoundError`, `lyric-stdlib/tests/regex_safe_tests.l` under `--target jvm`). Fixed by seeding the package-QUALIFIED extern-type key bundle-wide (unconditional of `wanted`/direct-import status), mirroring `ctx.ctors`'s existing bundle-wide qualified-key fallback for plain declared types — the bare (unqualified) key stays import-scoped, so the #5976 own-type-shadowing semantics are unaffected. Verified by the new transitive-extern-type case in `jvm_cross_package_collision_self_test.l` and the JVM CI step for `regex_safe_tests.l`. | `bridge.l`, `codegen/01_types.l`, `codegen/02_exprs.l`, `codegen/03_match.l`, `codegen/04_calls.l`, `codegen/05_stmts.l`, `codegen/06_items.l` | (D-progress-543), #5976 |
| m-21 | ~~`x.toString()` on a primitive receiver fell through unhandled (mis-stacked into string concat → `VerifyError`); `JByte` div/rem returned `JInt`; Java renders `1500.0` where .NET renders `1500` for whole Doubles, a formatting divergence~~ **Fixed (D-progress-543, Double divergence fixed in D-progress-664):** primitive receivers box and call `Object.toString`; the masked `JByte` div/rem branches now yield `JByte`, keeping later narrowing/type-tracking exact (#4551); `emitNormalizeDoubleString` strips a whole-valued Double's trailing `.0` at every stringification call site so the JVM backend matches .NET's rendering for the ranges the #4688 issue covered (#4688). **Residual gap (#5660):** Java's `Double.toString()` switches to scientific notation for `|value| >= 1e7` (or very small magnitudes), which .NET's default format does not at the same threshold, so whole Doubles above ~10 million still diverge (`"2.0E7"` vs `"20000000"`) — not addressed by this fix. | `codegen/02_exprs.l`, `codegen/04_calls.l` | #4551, #4688 ✅ (trailing-`.0` case), #5660 (scientific-notation residual) |
...
| m-90 | ~~`impl <ExternInterface> for Record` (docs/51's JDK-interface-implementation mechanism, shipped MSIL-only) was silently broken end-to-end on JVM: `constraintRefToJvmClass` resolved a single-segment `impl` interface name against the *local package* instead of the file's `extern type` table, so the emitted `implements` clause named a nonexistent in-package class (`NoClassDefFoundError` at class-load). Two sibling gaps in the same mechanism: `holderAwareParamTypes`'s `erase` branch and `lowerImplMethod`'s return-type resolution both used the plain (non-extern-aware) type-resolution helpers, degrading an extern-typed impl-method parameter or return value the same way. Separately, JVM auto-FFI had never resolved *interface*-typed extern types at all — `Jvm.ClassReader.parseClass` unconditionally skipped every `ACC_INTERFACE` class file, so an `extern type` naming a JDK/Maven interface (not a concrete class) always fell through to the untyped `(Object…)Object` guess, which the JVM verifier rejects for an interface owner.~~ **Fixed (D-progress-629, D-progress-631):** `constraintRefToJvmClass`/`holderAwareParamTypes`/`lowerImplMethod` now resolve through `externTypes` before falling back to the local-package guess (D-progress-629, `codegen/06_items.l`); `ClassInfo` gained an `isInterface` field and `lowerAutoFfiInstanceCall` emits `invokeinterface` instead of `invokevirtual` for an interface-owned method (D-progress-631, `class_reader.l`/`codegen/04_calls.l`), plus a recursive superinterface walk in `Jvm.AutoFfi.findBestInstanceMethod` (`scoreInterfacesRec`) needed for APIs whose methods live on extended sibling interfaces rather than the bound type itself (Lettuce's `RedisCommands`). Verified by `ffi_iface_impl_jvm_self_test.l` (`java.io.FilenameFilter`, 2 cases) and real end-to-end use in `lyric-web`'s Undertow `HttpHandler` impl, `lyric-ws`'s Undertow WebSocket listeners, `lyric-session`'s Lettuce Redis client, and `lyric-aws-xray`'s X-Ray `Entity` interface — zero regressions across `auto_ffi_jvm_self_test.l` (22 cases), `iface_dispatch_jvm_self_test.l` (3 cases), `bitwise_self_test.l`, and `aspect_weave_self_test.l`. | `codegen/{04_calls,06_items}.l`, `class_reader.l`, `auto_ffi.l` | #3334, #5445 ✅ |
| m-91 | ~~Interface default-method dispatch (`IMFunc`, an interface member with a body) lowered its body via `lowerFunc` (`ACC_STATIC`), while every call site registered the member `isIface = true` and always emitted `invokeinterface` against it — `IncompatibleClassChangeError: Expected instance not static method`. A second, independent gap in the same area: the `out`/`inout` holder-call path (`lowerVirtualCallWithHolders`) hard-coded `invokevirtual` unconditionally, never checking `sig.isIface` — so *any* interface method (abstract-then-`impl`'d too, not just a default body) called with `out`/`inout` params through an interface-typed binding crashed the same way, unreached by the pre-existing `out_inout_instance_jvm_self_test.l` (which only dispatches through the concrete record type).~~ **Fixed (D-progress-676):** `IMFunc` now routes through `lowerImplMethod` (the same instance-method codegen `impl` methods already use); `lowerVirtualCallWithHolders` (and the newly-reachable bare-sibling-call path in `lowerGeneralStaticCall`) branch on `sig.isIface` to select `invokeinterface`, mirroring the non-holder path. Verified by `iface_default_method_out_inout_jvm_self_test.l` (4 cases: default-method inout/out, abstract-then-impl'd inout, plain no-holder default method). | `codegen/{04_calls,06_items}.l` | #3604 ✅ |
| m-92 | ~~A non-`Unit` function whose body is a bare tail-position `try { } finally { }` (no explicit `return` inside the try) silently discarded the try body's value: `lowerBlockExprInner` fell back to statement-mode lowering whenever a tail `STry` carried a `finally` block, so `lowerFunc` saw `JVoid` and pushed the return type's zero/default instead of the real result — `func f(): Int { try { 42 } finally { cleanup() } }` returned `0`, not `42`, confirmed by direct `java -jar` repro before the fix. Not a `panic`/diagnostic gap; a genuine silent miscompile. The MSIL backend already handled this shape correctly via `lowerTryCatchExprMsil`'s value-join.~~ **Fixed (D-progress-677):** new `lowerTryCatchFinallyExpr` (`codegen/05_stmts.l`) combines the value-join machinery of `lowerTryCatchExpr` with the catch-all-then-rethrow finally strategy of statement-mode `lowerTryCatch`: the try body's (or a matching catch arm's) value is stashed to a result slot, the finally block always runs before both the normal-completion return and the exceptional rethrow, then the stashed value reloads. Verified by two new cases in `control_flow_jvm_self_test.l` (normal-completion and exceptional-path value survival). | `codegen/{02_exprs,05_stmts,06_items}.l` | #2043 ✅ |
| m-93 | ~~The `__lyricCount` synthesized erased-receiver `.count`/`.length` dispatch helper was emitted into every compiled JVM package unconditionally, even when nothing in the compilation unit called it — dead bytecode inflating every JAR.~~ **Fixed (D-progress-678):** a new `usesLyricCount: List[Bool]` flag threads through `FuncCtx` (mirroring the existing `closureAcc` pattern); the erased-receiver `.count`/`.length` dispatch site sets it, and `codegenPackageWithSigsSeeded` only emits the helper when the flag is set. Verified both directions by unzip+`strings` on compiled JARs (zero `__lyricCount` occurrences for a package that never uses it, exactly one for a package that does) and by `lazy_lyric_count_jvm_self_test.l` (4 cases). | `codegen/{01_types,02_exprs,06_items}.l` | #4803 ✅ |
| m-94 | The erased-receiver member-access refusal (an `Object`-erased receiver with no registered field/`.count`/`.length` fallback, `02_exprs.l`) and every other uncoded codegen panic escaped as an unhandled `Bug`/host stack trace, able to abort a whole `lyric test` run mid-batch. **Fixed (D-progress-754):** the refusal now self-tags `error[J007]: <dottedPkg>:<line>:<col>: …` (same convention as J004/J005); every OTHER codegen panic is caught at the `Lyric.Emitter` boundary and re-wrapped as `error[J008]: JVM codegen failed: …` (containment lives at the emitter, not `Jvm.Bridge`, so the bridge's own throw-on-refusal library contract stays intact for its in-process diagnostic self-tests). J007 stays a genuine refusal for a receiver whose concrete type is truly unknown at compile time (e.g. a `Map[K, V]` value read, #6347/#6357's root cause); it no longer fires for a non-receiver `Self`-typed impl param, which #6426/D-progress-755 narrows with a real `checkcast` instead. | `codegen/02_exprs.l`, `emitter.l` | #6422, #6357, #6347, #6426 |
| m-95 | A `Self`-typed interface/impl/record-method RETURN (`func dup(self: in Self): Self`) always erased to `java/lang/Object` at every call site — the JVM analog of the non-receiver `Self`-PARAM gap #6421/#6426 fixed, but for the RETURN position, which #6426's own investigation explicitly scoped out (D-progress-755) and #6431 tracked as a follow-up. A member access through the call result (`b.dup().n`) hit the erased-receiver refusal (J007). **Fixed (D-progress-756):** `narrowSelfCallResult` (`codegen/04_calls.l`) emits a `checkcast` to the receiver's own known concrete class right after a non-interface `invokevirtual` dispatch, narrowing the tracked call-result type from `Object` to that class, whenever the callee's registered sig carries a new `retIsSelf: Bool` field (set by `registerInstanceSigErased`/`06_items.l` for a bare-`Self`-declared return). Sound because `checkImplConformance` guarantees a `Self`-declared return yields exactly the receiver's own concrete type. An interface-typed receiver stays correctly erased and unreachable in practice: the type checker already types such a call's result as the INTERFACE ITSELF (T0113) before codegen ever sees it. Investigating this fix found MSIL's PRE-EXISTING return-position handling was NOT a genuine call-site substitution mechanism — it silently relied on the same package-wide field-NAME-only `fieldTokensByName` fallback D-progress-753 fixed for the param case, throwing `InvalidCastException` when two same-package records share a field name (#6434), and a chained call (`b.dup().dup()`) failed even without a collision (MSIL's `methodRetTypes` erased the tracked return type to `MObject` unconditionally, so the second call couldn't even find a dispatch target, #6435) — both real, previously-undocumented MSIL gaps, flagged at the time as out of THIS (JVM-only) fix's scope. **Both fixed on MSIL in D-progress-757**: a parallel `methodRetIsSelf: Map[String, Bool]` side table (keyed identically to `methodRetTypes`) records whether a method's RAW, pre-erasure declared return is bare `Self`; `narrowSelfCallResultMsil` (`msil/codegen.l`) consults it at MSIL's two reachable instance-call dispatch sites with a tracked receiver (the explicit-receiver `MClass(cls)` branch of `lowerMethodCallMsil`, and the bare intra-impl sibling-call branch of `lowerGeneralStaticCall`) and narrows the CODEGEN-INTERNAL tracked type to `MClass(cls)` — no `castclass` is emitted (unlike JVM), since the CLR JIT does not enforce CIL type-safety verification at load time (D-progress-753's own finding) and a later field read through the narrowed receiver already emits its own cast at the existing field-lowering site. MSIL needs no third, holder-path-specific site the way JVM's `lowerVirtualCallWithHolders` did — an MSIL `out`/`inout` argument is handled inline in the same `MClass(cls)` dispatch block. The former JVM-only `self_return_call_site_jvm_self_test.l` (6 cases) was promoted into the dual-target `bare_func_ref_self_test.l` (24 -> 30 cases, both targets) and deleted, now that both targets pass. `narrowSelfCallResult` (JVM) also gained a defensive `sig.isIface` early-out per the #6436 review disposition, making its "callers pass this only for a non-interface dispatch" convention self-enforcing. | `codegen/{01_types,04_calls,06_items}.l`, `bridge.l` (JVM); `msil/codegen.l` (MSIL) | #6431, #6434, #6435 |
| m-96 | The three sibling `EMember` "member cannot be resolved" panics next to J007's erased-receiver refusal (known-class-but-no-such-field, array receiver, primitive receiver, `02_exprs.l`) stayed unprefixed, surfacing as the generic `error[J008]: JVM codegen failed: …` catch-all since #6422's fix rather than their own stable codes. **Fixed (D-progress-756):** minted dedicated codes following the exact J007 pattern — `error[J009]` (known-class-no-field), `error[J010]` (array/slice receiver), `error[J011]` (primitive receiver). All three are reachable from checker-legal source, not just defensive: the type checker's `symTableIsMemberComplete` gate only tracks record/interface types DECLARED IN THE CHECKING FILE, so a member access on any cross-package type, `extern type`, `slice[T]`, or primitive receiver skips T0113 entirely (`maybeUnknownMemberDiag`'s early return, or the blanket `case _ -> TyError` for non-`TyUser` receivers) — verified with a `JStringBuilder.new(...).bogusField`-shaped repro for J009 (`jvm_trycatch_bridge_self_test.l`'s new case) and standalone CLI repros for J010/J011. | `codegen/02_exprs.l` | #6427 |
| m-97 | #3432 ("no generic-declaring-type `@externTarget` member emission, MSIL parity gap"): MSIL's `emitGenericExternMember` (closed GENERICINST TypeSpec + `!0`/`!1`-encoded member signature) has no JVM analog. Investigating found the JVM design is genuinely different, not merely unported: JVM erasure means an instantiation of a generic JDK type (`ConcurrentHashMap<K,V>`) is always the SAME raw class file, so `Resilience.Kernel.Jvm`/`Ws.Kernel.Jvm`/`Jobs.Kernel.Jvm` already sidestep the whole problem by declaring the RAW (non-generic) `extern type` and dispatching ordinary member calls through it (D-progress-588) — no TypeSpec-construction analog is needed. Only `Web.Kernel.Jvm`'s rate limiter used the generic-alias form (`JvmConcurrentDict[K, V]`, needed because `newConcurrentDict[K, V]()`'s ctor and `cdTryGetValue[K, V]`/`cdPut[K, V]`'s member calls are themselves plain GENERIC Lyric functions monomorphized per call site, not `@externTarget` members) — and, while `typeExprToJvmExtern`'s `TGenericApp` arm already resolves that generic alias to its raw erased class (#5458, D-progress-663), a genuine, broader, previously-undetected bug sat one level deeper: **`cdTryGetValue`'s `out V` parameter never worked once monomorphized on JVM at all**, generic-extern or not — `Jvm.Bridge.collectMonoSpecializedSigs` (patches the cross-package call registry with a monomorphizer specialization's REAL signature, since sigs are collected pre-mono, m-25) hand-rolled its own params/modes loop instead of reusing `holderAwareParamTypes`, always recording `paramModes = newList()` (empty) and a plain (non-array) param type for every specialized function — so ANY generic function with an `out`/`inout` parameter (`cdTryGetValue[K, V]`, and any similar helper, generic-extern-typed receiver or not) compiled a call site that passed the raw value where the specialized method's OWN definition (correctly holder-array-typed via the normal `06_items.l` registration path) expected a one-element holder array: `NoSuchMethodError` at runtime, on every call, never a build-time failure. Confirmed with a minimal non-extern repro (`func setOut[V](x: in V, value: out V): Bool` specialized to a record `V`) before narrowing to the exact `cdTryGetValue` shape. **Fixed:** `collectMonoSpecializedSigs` now calls `holderAwareParamTypes(decl.params, owner, false, newList(), externTypes)` for `params` and threads `decl.params[pi].mode` into `paramModes`, matching every other registration site. Also added, per the issue's point 2: `Jvm.Codegen.lowerExternTargetBody`'s `"<init>"` (ctor) branch ran NO metadata verification at all (unlike the static/instance branch's F0015-J check) — a mistyped ctor `@externTarget` signature silently compiled and only failed at runtime with `NoSuchMethodError`. It now calls `verifyExternTargetJvm(ctx.autoFfi, targetFqnDot, "new", allParamDescs, true)` (reusing the existing ctor-aware `findBestConstructor` path `verifyExternTargetJvm` already had for the `.new()` constructor-shorthand call site) and panics with a ctor-specific F0015-J message on a real mismatch; the wrapper's declared RETURN type is never compared against `<init>`'s `void` — it is reconciled by construction, since the emitted shape always returns the freshly `new`'d instance. Verified end-to-end: a new JVM analog of `generic_extern_self_test.l` (`generic_extern_jvm_self_test.l`, ctor + put/get + overwrite + absent-key + Int-key round-trip over a generic-`extern-type`-aliased `ConcurrentHashMap`, 5 cases) passes under `lyric test --target jvm`; a standalone repro of `Web.Kernel.Jvm.checkRateLimit`'s exact tumbling-window/burst logic over `newConcurrentDict`/`cdTryGetValue`/`cdPut` now produces the correct allow/deny counts under real `java`; `lyric-resilience`'s full JVM suite (`lyric test --manifest lyric-resilience/lyric.toml --target jvm --no-default-features --features jvm`, 17 cases — circuit breaker + backoff) passes 17/17 (it already worked pre-fix, via the raw-erased-type pattern, confirming D-progress-588's fix holds); zero regressions across `out_inout_jvm_self_test.l` (18), `out_inout_instance_jvm_self_test.l` (9), `iface_default_method_out_inout_jvm_self_test.l` (4), `stdlib_generic_mono_self_test.l` (7), `map_iteration_jvm_self_test.l` (5), `auto_ffi_jvm_self_test.l` (39), `bitwise_self_test.l` (10), and `aspect_weave_self_test.l` (8). | `jvm/bridge.l`, `jvm/codegen/04_calls.l` | #3432 |
| m-98 | An unannotated homogeneous list literal (`val xs = [9, 7]`) always built a plain `ArrayList` with no element-type tracking (`case EList` unconditionally returned `JRef("java/util/ArrayList")`), so an indexed read erased to `Object` and a following relational/arithmetic use (`xs[0] < xs[1]`) panicked loudly with "unsupported reference comparison op … on operands of reference type 'java/lang/Object'" — the JVM counterpart of MSIL's #5620 (fixed by building a genuine `T[]` there), found while implementing that fix. **Fixed (#5686, D-progress-803):** rather than adding a new `JvmType` representation (which would need threading through every one of the ~700 existing match arms that dispatch on a list/collection receiver's static type — `.add`/`.count`/iteration/auto-FFI/etc. — for a change that only the indexed-read narrowing path needs), `inferHomogeneousListElemTypeJvm`/`listLiteralElemTypeJvm`/`isSimpleListElemJvm` (`codegen/02_exprs.l`, mirroring MSIL's `inferHomogeneousListElemTypeMsil`) peek a literal's elements for one shared simple type (from a literal's own kind, or a bare name's tracked `ctx.types` entry), and `scrutineeGenericArgs`'s new `EList` arm (`codegen/03_match.l`) records the peeked type through the SAME `ctx.varGenericArgs` registry an explicit `List[Int]` annotation already populates — so the pre-existing `indexedElemTypeOverride`/`applyIndexedElemOverride` narrowing (docs/59 §4.3) picks it up unchanged. The `ArrayList` runtime representation is untouched; only the compiler's own type tracking gained the extra inference. `list_literal_index_self_test.l` (shared with the MSIL fix) now runs on both targets (6 cases: Int/String/Double/Bool/Char/Long/Byte element types, plus a `[a, b]`-over-locals case), wired into `compiler-self-tests-jvm`. | `codegen/{02_exprs,03_match}.l` | #5686 ✅ |
| m-99 | `Map[K, V]` VALUE reads through `m[k]` erased to `Object` unconditionally — `recordDeclaredElemType`/`registerFieldElemSig` (the #5439/#5444 declared-element-type registries) explicitly skipped `Map`, and the `HashMap.get` `EIndex` arm computed but discarded its `indexedElemTypeOverride` result — so any typed use of a Map value (a field read, an instance method) either J007-refused at compile time or (pre-#6422) silently miscompiled at runtime, and a `Map[String, Record]` sharing a JVM bundle with an unrelated `Map[String, Long]` could misresolve against the wrong class (`lyric-session` `InProcessSessionStore.set()`, #5451). **Fixed:** `recordDeclaredElemType`/`registerFieldElemSig` gained a `Map`-with-2-args arm recording `V` (index 1) alongside the existing 1-arg slice/List convention (index 0); `indexedElemTypeOverride`'s general arg-count check now also consults a 2-element entry; the `HashMap.get` `EIndex` arm applies the resolved override via `applyIndexedElemOverride`, exactly like the `ArrayList.get`/array arms already did. **Scoping bug found and fixed during development:** the SAME `indexedElemTypeOverride` helper backs `SFor`'s loop-iterable narrowing (`05_stmts.l`), where a 2-arg return type does NOT reliably mean "Map's `V` at index 1" — `Std.CollectionsHost.MapKeyCollection[K, V]` (`java.util.Set`, aliased with 2 phantom type params for symmetry with `Map[K, V]`) iterates `K`, not `V`; blindly applying the Map heuristic there silently miscompiled `Std.Collections.mapKeys`'s own `for k in dictGetKeys(m)` loop (a String key checkcast to Integer). Gated behind a new `allowMapValueArg: Bool` parameter: `true` at both `EIndex` call sites (a receiver the type checker only ever lets `[k]`-subscript is `Map[K, V]`, unambiguous), `false` at the `SFor` call site (preserves the pre-existing, safe single-arg-only behaviour there). | `codegen/01_types.l`, `codegen/02_exprs.l`, `codegen/06_items.l` | #5451 (root-causes half of #6347/#6357, whose containment-only fix in D-progress-754 explicitly left this generics-erasure gap open) |
...
- **M-19 (DONE, D-progress-625):** `_kernel_jvm/environment_host.l` /
  ...
  without new entry-point codegen)~~ **Fixed:** `Jvm.Codegen`'s `hasMain`
  entry-point synthesis now stashes argv into a package-independent
  holder class (`__LyricJvmRuntime`) before forwarding to the Lyric
  `main`, unconditionally (works even when `main()` declares no `argv`
  parameter); every `hostGetCommandLineArgs` call site is lowered
  directly to a `getstatic` read of it. Verified with a `main()` that
  takes no argv parameter (`lyric-compiler/jvm/env_args_jvm_main.l`).
  #5378 (`Never`-tail-expression
...
- M-18: `lyric run --target jvm` (#674) ships; `main(args: slice[String]): Int`
  ...
  `_kernel_jvm/time_host.l` rewrite. GraalVM native-image (#675/#1975) shipped
  in D131 / D-progress-630: `lyric build --release --aot --target jvm` drives
  `native-image` over the bundled JAR (`--no-fallback`, so a JVM-requiring
  fallback image is a build failure, not a silent downgrade) on Linux and
  macOS; Windows remains open. **M-18 is closed.**
...
distribution (#674 ✅, #675 ✅, #1975 ✅ on Linux/macOS, #680 ✅), F#-host elimination (`docs/41` H12,
