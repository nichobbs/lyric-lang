...
| m-18b | ~~`Std.Collections` Map iteration helpers (`mapKeys`/`mapValues`/`mapEntries`/`mapForEach`/`mapPutAll`) had no JVM kernel; `tryGetValue`/`setToSlice` referenced phantom `lyric.stdlib.jvm.*` shim classes~~ **Fixed (D-progress-542):** `_kernel_jvm/collections_host.l` binds `dictGetKeys`/`dictGetValues` to `HashMap.keySet()`/`.values()` (both `Iterable`, iterated by the emitter's Iterable path); `tryGetValue` and `setToSlice` are plain Lyric over `containsKey`/`get` and `HashSet.toArray()`; `newListWithCapacity` added. Verified by `map_iteration_jvm_self_test.l`. | `_kernel_jvm/collections_host.l` | #3676 |
