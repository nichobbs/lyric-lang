...
The `var` prefix is accepted by the parser, which carries it into `FieldDecl.isMutable`. The mode checker (`lyric-compiler/lyric/mode_checker/`) enforces it for the shape it can resolve without a symbol table: a write to `r.field` (or `r.field op= …`) after construction, where `r` is a `var` local of a record type declared in the same file and `field` is not marked `var`, is rejected with **V0015** ("cannot reassign immutable field"). Construction (`Rec(field = x)`) is never flagged. This is deliberately narrow (issue #1815 part 1): write access through an `inout` parameter is not tracked (a pervasive stdlib idiom threads a cursor-style state record through `inout` helpers that mutate fields directly, e.g. `XmlState.pos` in `Std.Xml` — tracking `inout` here would flag the existing stdlib), nor is a write through an immutable (`val`/`let`/`in`) binding, nor a write through a type declared in a different file. Emitter-level enforcement (`initonly`/`ACC_FINAL` so the field is also locked at the IL/bytecode level, and widening the mode-checker rule to `inout` parameters and cross-file types) is tracked as follow-up work (issue #1815 part 2). The syntax is intentionally similar to local `var` declarations so that the intention is clear in code review.
...
| ~~`"aot"`~~ | **removed** — use `[build] shape = "aot"` (§13.1); declaring it is `F0035` | — | — |
...
| `"standalone"` | runtime bundled, still JIT | `./<name>` | none — *not implemented (`F0037`, #6262)* |
...
**`--release` no longer implies AOT.** Before the profile/shape split it selected a packaging mode; it now selects the optimization profile only, and `--aot` (or `[build] shape = "aot"`) is required for a native binary. A `--release` build that resolves to a non-`aot` shape and named no shape on the command line prints a one-line note saying so. Diagnostics: `F0033` (`--debug` and `--release` together), `F0034` (conflicting shape spellings, e.g. `--aot --shape portable`), `F0035` (`[build] kind = "aot"`, removed — use `shape = "aot"`), `F0036` (shape invalid for the target, e.g. `--target native --shape portable`, since a native build is AOT by construction), `F0037` (shape valid but its toolchain path is unimplemented — `standalone` on every target today, #6262). `--define` and `--watch` are rejected on a non-`portable` shape rather than on `--release`: it is the packaging path that threads no defines and is too slow to watch, not the profile.
