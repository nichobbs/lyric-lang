...
**Property execution — `--properties`.** By default a `property "<title>" forall(<binders>) [where <expr>] { <block> }` declaration still parses but skips at runtime (`# skip` line), as in v1. Passing `lyric test --properties` runs each property alongside ordinary tests instead: when every `forall` binder is one of the auto-derived primitive types (`Int`, `Bool`, `Double`), the runner samples 100 random inputs (seeded per property for reproducible runs), evaluates the optional `where` guard before each sample (a guard failure is not a counterexample — the sample is skipped, not counted as pass or fail), and on the first failing sample shrinks each binder toward its simplest still-failing value (holding the others fixed) before reporting `not ok` with the shrunk counterexample in the failure message (e.g. `property failed: "insert then contains" forall(x = 0, initial = ...)`). A property whose `forall` binders include any other type still reports `# skip`, naming the unsupported type, both with and without `--properties` — v1's "parses but never runs" behaviour is unconditionally preserved for those binder shapes rather than silently attempting (and mis-generating) an unsupported generator. `--properties` is rejected up front on `--target native` (no exception unwinding to isolate a failing sample without aborting the whole binary). See `docs/24-test-runner-plan.md` §5 for the design and its v1.x follow-ups (custom generators, `--seed`, contract-derived properties).
