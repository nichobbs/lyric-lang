...
- **Structural (V0014)** — **shipped** (slice S2, D-progress-597): the shared front-end rejects a `spawn` used as a discarded statement outside a `scope`, closing the canonical fire-and-forget hole on all three backends. Native additionally enforces the value-position case (D-N-022). Full consumption analysis (unused spawn bindings) is a follow-up.
