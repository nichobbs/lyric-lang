...
Unresolved calls, narrowing coercions, or instance methods on a value-type receiver fall back to requiring an explicit `@externTarget` wrapper. All unresolved auto-FFI calls, member reads, and property assignments on extern receivers produce a compile-time diagnostic — never a silent mis-lowering.
