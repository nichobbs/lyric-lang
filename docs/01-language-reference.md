...
When `e` is a direct call to an `async func` — no literal `await` written — `e?` awaits it in place before matching, exactly as if `(await e)?` had been written; this holds with or without the `await` keyword (`--target dotnet`; see `Lyric.Propagate`'s implicit-`EAwait` rewrite, docs/03 D-progress-941). Inside an `async func`, that implicit await is rejected with `F0045` when the `?` sits lexically inside one of the function's own `try`/`catch`/`finally` regions and the scrutinee cannot be proven non-async (a same-file call known to be `async`, a cross-package or method call, an unresolved name, or a composite `if`/`match`/value-position-`try` scrutinee whose branches could themselves be async calls) — the CLR verifier rejects a suspend/resume sequence physically inside a protected region, so the compiler declines rather than emit invalid IL; write the `await` explicitly before or after the `try` block instead, matching V0012's identical rejection of an explicit `await` in the same position.
