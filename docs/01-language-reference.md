...
incomplete task, or a `Std.Time.sleepMillis` call inside an async
body, which parks only the calling task instead of blocking the
thread). A direct call to an async function awaits in place with or
...
This is the structured concurrency pattern; raw "fire and forget" is not available: a `spawn` is legal only lexically inside a `scope { }` (or a function whose entire body is a `scope`, by sugar), and a `spawn` elsewhere is a compile error (**V0013**), so a spawned task cannot outlive its scope.
...
the runtime's cooperative scheduler (`Std.Time.sleepMillis` inside an
async body is the async leaf — it suspends only the calling task).
