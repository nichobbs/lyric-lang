...
**`--target native` (D-N-019):** a non-generator `async func` and `await`
are supported. `Task[T]` is not materialised as a distinct value on
native — with `spawn`/`scope` (§7.4) out of scope for this slice, there
is no way to hold an async call's result unawaited, so a non-generator
`async func` body compiles through the identical codegen path as a plain
`func` and `await expr` is a pure passthrough (calling an async function
with or without `await` yields its value the same way). Async generators
(`yield` inside `async func`, §7.2) are rejected with a dedicated
diagnostic; the implicit `cancellation` parameter (§7.3) and `spawn`/
`scope` structured concurrency (§7.4) are not yet supported.
...
**`--target native` (D-N-021):** `spawn` and `scope { }` are supported as
the same passthrough model the MSIL emitter itself uses (`spawn expr`
evaluates `expr`; `scope { }` is a lexical block whose ARC releases and
`defer` blocks run at scope exit). Native has no async leaf primitive
(no async timer or I/O kernel), so a spawned call always completes at
the spawn site and the guarantees above hold degenerately — the same
degenerate form .NET semantics take when restricted to the
native-expressible surface. Genuine concurrent progress arrives with the
first async leaf kernel, the tracked follow-up at which the LLVM
coroutine mechanism preserved in D-N-019 becomes load-bearing.
