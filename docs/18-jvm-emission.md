...
The release path builds one *bundled* JAR (user package + transitive
`Std.*` closure + sibling packages), so there is a single config set to
aggregate, and a staging directory keeps it out of the shipped artifact.
Per-package embedded configs remain the right shape for **library** JARs
consumed by a non-Lyric native-image build; that is not yet emitted.

The generators live in `Jvm.NativeImage` (`lyric-compiler/jvm/native_image.l`):

- `reflectConfigJson()` — always `[]` (no reflection in Lyric-generated
  code).  Java libraries that need reflection ship their own configs,
  which native-image aggregates from their JARs at link time.
...
All four schemas are **top-level JSON arrays**.  The object forms
(`{"jni": []}`, `{"proxies": []}`) are a hard config-parse error, and
native-image reports it inside its build-summary block, still prints
`Finished generating '<name>'`, and exits **0** without writing a
binary — so the driver treats "output file exists" as a real check.

The Lyric build driver invokes `native-image` directly with
`--no-fallback`, which makes an unclosable world a build failure
instead of a JVM-requiring fallback image.
...
`bootstrap/src/Lyric.Emitter/`); when the F# bootstrap port ships
...
| B111  | `lowerSealedUnion` | §8.1 production form | Shipped |
| B112  | `lowerEnum` | §8.2 | Shipped |
| B113  | `lowerOutInoutParam` helpers | §11.2 | Shipped |
| B114  | `lowerNatTag` | §10.5 | Shipped |
| B115  | `makeLyricSignatureAttr` | §10.4 | Shipped |
| B116  | `lowerExposedRecord` | §9 | Shipped |
| B117  | `lowerProjectable` | §7.3 | Shipped |
| B118  | `lowerProtectedWithBarriers` | §17.3 | Shipped |
| B119  | `lowerHotAsync` | §14.2 | Shipped |
| B120  | `lowerScopeBlock` | §15 | Shipped |
| B121  | `lowerFuncWithContract` | §20 | Shipped |
...
Java libraries from Maven Central, analogous to NuGet linking on the
.NET side.  Design questions include: whether to shell out to `mvn`
or bundle a resolver; how to derive a collision-free Lyric package
name from a `groupId:artifactId` coordinate; and how to map Java
...
**Resolution (#3932, D-progress-882):** the JVM backend implements the D107
...
| Q-J010 | `@externTarget` `Option[T]` null-coercion (D107 Phase 2) | §24: **RESOLVED** — shipped in D-progress-882 (#3932) |
...
`bootstrap/src/Lyric.Cli/SelfHostedJvm.fs` bootstraps `Jvm.Bridge.dll` via a
