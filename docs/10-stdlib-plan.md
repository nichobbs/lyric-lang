...
| `Std.Collections.Persistent` (`collections_persistent.l`) | `@experimental` | Immutable (never-mutating) list operations extending D038's native-stdlib direction (#684): `plistCons`/`plistHead`/`plistTail`/`plistLookup`/`plistInsert`/`plistDelete`/`plistToList`/`plistFromList` operate on a bare `slice[T]` (no wrapper type needed — `slice[T]` is already immutable at the language level, mirroring `Std.Sort.sort[T]`'s bare-`slice[T]` convention); `plistHead`/`plistLookup` are O(1), the rest rebuild the slice, O(n). Pure Lyric, no BCL/JVM extern boundary — cross-target by construction (CI-verified on both `--target dotnet` and `--target jvm`, #6577). `@experimental`, not `@stable`, despite that verification — the module is brand new and its own header discloses two self-hosted MSIL generic-codegen bugs (#6568, #6569) found in adjacent code while building it; promotion to `@stable` awaits real-world usage, matching the `mapSize`/`mapIsEmpty` convention above. A persistent `Map` (and, as a stretch goal, `Set`) were planned but are **deferred, not shipped** — tracked in #6570, blocked on two reproducible self-hosted MSIL backend bugs found while building them, each filed with an isolated repro: #6568 (a union/record case field typed as the enclosing generic type's own open instantiation — a classic cons-list/BST node — or as a container of the enclosing type's own parameter corrupts field reads at runtime) and #6569 (a generic function whose body invokes a closure parameter inside a loop conditional over a `slice[UserGenericRecord[K, V]]` parameter — the natural shape for a comparator/equality-keyed map — monomorphizes with its first parameter erased to `Object`, crashing with `NullReferenceException` on the very first call). Full detail is in the module's header doc comment. Tests: `lyric-stdlib/tests/collections_persistent_tests.l` (empty-list edge cases, all list operations, non-mutation assertions). |
