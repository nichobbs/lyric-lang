...
| `Std.Collections.Persistent` (`collections_persistent.l`) | `@experimental` | Immutable (never-mutating) list AND map operations extending D038's native-stdlib direction (#684). List: `PersistentList[T]` is a linked cons list (`PLNil`/`PLCons(head, tail, size)`, D-progress-980, #7280) — `plistEmpty`/`plistCons`/`plistHead`/`plistTail`/`plistLength`/`plistIsEmpty` are O(1) and share the list they derive from, `plistLookup`/`plistInsert`/`plistDelete` are O(index), copying only the nodes before the index, and `plistToList`/`plistFromList` are O(n). (It was a bare `slice[T]` until #7280, which made every cons and tail a full copy.) Map (#6570, shipped): `pmapIsEmpty`/`pmapSize`/`pmapLookup`/`pmapContainsKey`/`pmapInsert`/`pmapDelete`/`pmapToList`/`pmapFromList` operate on a bare `slice[MapEntry[K, V]]` (an association list keyed by a caller-supplied `eq: in (K, K) -> Bool` predicate — Lyric has no generic `Eq`/`Ord` interface to dispatch on; a naive-but-correct v1, not a balanced BST, per the original #684 scope); `pmapSize`/`pmapIsEmpty` are O(1), the rest are O(n); `pmapInsert` replaces an existing key's value in place (keeps its position) rather than growing the map. Pure Lyric, no BCL/JVM extern boundary. List ops are cross-target by construction (CI-verified on both `--target dotnet` and `--target jvm`, #6577); Map ops are now **also verified on both targets** — a separate JVM-backend generics-erasure gap (#6708, found while building Map, unrelated to #6568/#6569) that failed field access on an element indexed out of a generic function's returned `List[MapEntry[K, V]]` (or a `slice[MapEntry[K, V]]` parameter, the shape `pmapLookup`/`pmapInsert`/`pmapDelete` actually use) is fixed. `@experimental`, not `@stable` — the module is brand new; promotion to `@stable` awaits real-world usage, matching the `mapSize`/`mapIsEmpty` convention above. Building Map surfaced (and fixed, in the same change) two previously-unknown self-hosted **MSIL** generic-codegen bugs with no prior working precedent anywhere in the codebase, each now covered by a dedicated `lyric-compiler` self-test: #6568 (a union/record case field typed as the enclosing generic type's own open instantiation — a classic cons-list/BST node — or as a container of the enclosing type's own parameter corrupted field reads at runtime; fixed by reifying the enclosing type's own arity before its field loop runs, and by giving `slice[T]`/`List[T]`/`Map[K,V]` container fields the same `MTypeVar`-carrying VAR-form treatment a bare `T` field already had) and #6569 (a generic function whose body invokes a closure parameter inside a loop conditional over a `slice[UserGenericRecord[K, V]]` parameter monomorphized with its first parameter erased to `Object`; found already fixed by unrelated prior work when re-verified against a from-source `main` build). Full detail is in the module's header doc comment. Tests: `lyric-stdlib/tests/collections_persistent_tests.l` (List ops: empty-list edge cases, all operations, non-mutation assertions; both targets) and `lyric-stdlib/tests/collections_persistent_map_tests.l` (Map ops: empty-map edge cases, all operations including overwrite/no-op-delete, non-mutation assertions; both targets, #6708). `Std.Collections.Persistent`-specific MSIL-backend regressions for #6568/#6569 are pinned in `lyric-compiler/lyric/inbundle_generics_self_test.l` and `lyric-compiler/lyric/generic_closure_container_self_test.l`. `PersistentSet[T]` remains an unshipped stretch goal. |
...
| `Std.Tls` (`tls.l`) | `@stable` | PEM certificate/private-key loading (docs/61 §3.1, D128, phase 1.1 of epic #5874): `Certificate`/`Identity` opaque types, `TlsVersion` enum, `TlsServerConfig` record (mTLS-ready from v1 per D128 decision 5), `TlsError` union; `Certificate.fromPemFile`/`fromPem`, `Identity.fromPemFiles`/`fromPem`. Backed by `Std.TlsHost` (`_kernel/tls_host.l` / `_kernel_jvm/tls_host.l`); only unencrypted PKCS8 private keys are supported (PKCS1/SEC1/encrypted keys are `Unsupported` on both targets, kept in parity deliberately). |
| `Std.HttpEngine` (`http_engine.l`) | `@experimental` | Pure-Lyric, sans-IO HTTP/1.1 protocol engine (docs/61 §6.1, D128, phase 3.2 of epic #5874, D-progress-694): no externs, no kernel file. `feed(conn, bytes): FeedResult` incrementally parses request bytes into typed events (`NeedMore`, `RequestHead`, `ExpectContinue`, `BodyChunk`, `RequestComplete`, `ProtocolError`); `serializeResponseHead`/`serializeChunk`/`serializeLastChunk`/`serializeFixedLengthBody` produce response bytes. `EngineLimits` (incl. `maxBodyBytes`), `HeaderField`, `Connection`, `ProtocolErrorKind`/`protocolErrorStatus`. Strict RFC 9112 framing (rejects `Content-Length` + `Transfer-Encoding` together, non-final/absent `chunked`, obs-fold, bare CR/LF, missing/duplicate `Host`, oversized bodies, etc. — see the module's own header for the full rationale). `@experimental` because no transport (#5884/#5882) drives it in production yet; the exhaustive (59-case) `lyric-stdlib/tests/http_engine_tests.l` corpus passes identically on both targets. |
