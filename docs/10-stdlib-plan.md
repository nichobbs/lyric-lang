...
| `Std.Collections.Persistent` (`collections_persistent.l`) | `@stable` | Immutable (never-mutating) list operations extending D038's native-stdlib direction (#684): `plistCons`/`plistHead`/`plistTail`/`plistLookup`/`plistInsert`/`plistDelete`/`plistToList`/`plistFromList` operate on a bare `slice[T]` (no wrapper type needed — `slice[T]` is already immutable at the language level, mirroring `Std.Sort.sort[T]`'s bare-`slice[T]` convention); `plistHead`/`plistLookup` are O(1), the rest rebuild the slice, O(n). Pure Lyric, no BCL/JVM extern boundary — cross-target by construction. A persistent `Map` (and, as a stretch goal, `Set`) were planned but are **deferred, not shipped**: hand-probing the self-hosted MSIL backend while building them surfaced two reproducible generic-codegen bugs with no working precedent anywhere in this codebase — (1) a union/record case field typed as the enclosing generic type's own open instantiation (a classic cons-list/BST node) or as a container of the enclosing type's own parameter corrupts field reads at runtime, and (2) a generic function whose body invokes a closure parameter inside a loop conditional over a `slice[UserGenericRecord[K, V]]` parameter (the natural shape for a comparator/equality-keyed map) monomorphizes with its first parameter erased to `Object`, crashing with `NullReferenceException` on the very first call. Full detail and isolated repros are in the module's header doc comment. Tests: `lyric-stdlib/tests/collections_persistent_tests.l` (empty-list edge cases, all list operations, structural-sharing assertions). |
