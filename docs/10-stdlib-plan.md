...
| `Std.HttpEngine` (`http_engine.l`) | `@experimental` | Pure-Lyric, sans-IO HTTP/1.1 protocol engine (docs/61 §6.1, D128, phase 3.2 of epic #5874, D-progress-692): no externs, no kernel file. `feed(conn, bytes): FeedResult` incrementally parses request bytes into typed events (`NeedMore`, `RequestHead`, `ExpectContinue`, `BodyChunk`, `RequestComplete`, `ProtocolError`); `serializeResponseHead`/`serializeChunk`/`serializeLastChunk`/`serializeFixedLengthBody` produce response bytes. `EngineLimits`, `HeaderField`, `Connection`, `ProtocolErrorKind`/`protocolErrorStatus`. Strict RFC 9112 framing (rejects `Content-Length` + `Transfer-Encoding` together, non-final `chunked`, obs-fold, bare CR/LF, etc. — see the module's own header for the full rationale). `@experimental` because no transport (#5884/#5882) drives it in production yet; the exhaustive `lyric-stdlib/tests/http_engine_tests.l` corpus passes identically on both targets. |
