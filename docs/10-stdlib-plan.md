...
| `Std.Regex` (`_kernel/regex.l`) | `@stable` | Regex compile + isMatch / matchOne / replace.  Compiled patterns carry a 1-second match timeout by default (`Regex(string, RegexOptions, TimeSpan)` overload) — adversarial input throws `RegexMatchTimeoutException` instead of hanging.  Use `compileWithTimeout` to override.  See #330. |
