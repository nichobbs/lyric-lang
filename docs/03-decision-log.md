...
- **#5620 (MSIL, silent miscompile):** an unannotated homogeneous list literal
  ...
  (`inferHomogeneousListElemTypeMsil`) and, when they share one simple type,
  builds a genuine `e[]` array — the checker types a bare list literal as
  `slice[e]` (`EList` → `TySlice`), so the array is the representation-correct
  form (this also keeps `xs.concat([4, 5])` working, where the literal must be a
  slice, not a `List`). Element types are peeked for literals and bare
  local/param names (`[a, b]`), covering the common shapes; elements whose type
  needs call/name resolution (`[f(), g()]`, member access) still fall back to
  the legacy path — a disclosed residual tracked in **#5690**. dotnet-only; JVM
  has the same root cause in a separate path (#5686).
...
`initialize` response's `serverInfo.version`) always printed the literal
`"0.1.0"` regardless of the actual release being shipped — every published
binary, whatever git tag it was built from, reported the same placeholder.
`sdk-version.json`'s `compiler_version` field (D109) inherited the same
staleness since it's generated by running `./bin/lyric --version` and
parsing the output. This was pure oversight: `.github/workflows/publish.yml`
already resolves a real semver per release (`create-release` job) and
threads it into every *other* build artifact (`-p:Version=` for the AOT
binary, `-p:PackageVersion=` for the NuGet tool, `--package-version` for
every ecosystem library's `lyric build`/`lyric publish`) — it just never
reached the compiler's own self-description.

**Decision.** Reject source-text stamping (rewriting the literal in a
`.l` file before `bootstrap.sh` compiles it) in favour of threading the
version through as a genuine build parameter, consistent with the
`--package-version` precedent already used for ecosystem libraries:

- `lyric-compiler/lyric/version.l` (`Lyric.Version.VERSION()`) reads
  `Std.Environment.getVarOrDefault("LYRIC_BUILD_VERSION", "0.1.0")` — a
  runtime env-var read with the historical placeholder as the dev-checkout
  fallback, not a literal.
- `bootstrap/src/Lyric.Cli.Aot/Lyric.Cli.Aot.csproj` gains a
  `GenerateLyricBuildVersion` MSBuild target (`BeforeTargets="CoreCompile"`)
  that writes a tiny generated C# file (`LyricBuildVersion.g.cs`, via
  `WriteLinesToFile`) declaring `internal const string Value = "$(Version)"`.
  A plain `ldstr` constant was chosen over reading
  `AssemblyInformationalVersionAttribute` via reflection specifically
  because `Lyric.Cli.Aot` publishes via Native AOT with trimming
  (`PublishAot=true`) for the standalone binary — a compiled-in literal
  survives trimming unconditionally, where attribute reflection is not
  guaranteed to.
- `Program.cs` (the pure trampoline, contract #1082) sets
  `LYRIC_BUILD_VERSION` from that constant via
  `Environment.SetEnvironmentVariable` immediately before its one
  in-process call into `Lyric.Cli.Program.main(args)` — relaying build
  metadata the self-hosted CLI has no other way to see, not new
  path-discovery or domain logic.
- The csproj defaults `<Version>0.1.0</Version>` when no `-p:Version=`
  override is supplied, so a plain `make lyric` / `dotnet build` keeps
  reporting the historical placeholder unchanged.
- `.github/workflows/publish.yml`'s NuGet-pack step gained `-p:Version=`
  alongside its existing `-p:PackageVersion=` (previously only the latter
  was set, which drives the `.nupkg` version but not `$(Version)`/the
  generated constant). The AOT-publish step already passed `-p:Version=`.
- The NuGet global-tool smoke test now asserts `lyric --version` prints
  exactly `lyric <release-version>` (previously it only asserted the
  command exited 0 and separately smoke-ran a compiled program) — a
  stamping regression now fails the release instead of shipping silently.

**Rejected alternative.** An earlier iteration of this fix
(`scripts/stamp_version.py`, run as a `publish.yml` step immediately after
checkout, before `bootstrap.sh`) rewrote the literal in `version.l`
in-place via a regex substitution, guarded by a self-test asserting the
regex matched the file's exact shape. Discarded before merging: it worked,
but was a bespoke text-substitution step bolted onto CI, disconnected from
the `-p:Version=`/`--package-version` mechanism already used everywhere
else in this pipeline, and fragile to any future reformatting of
`version.l`.

**Ecosystem libraries — no gap found.** While scoping this, confirmed
`lyric build --package-version X` / `lyric publish --package-version X`
(`cli_build.l:1284`, `cli_publish.l:133`, already wired into
`publish.yml`'s "Build and publish ecosystem libraries" step) already
correctly overrides each library's manifest-declared version in the built
assembly, embedded contract metadata, and published `.nupkg` — this was
already a production-quality build-time mechanism requiring no fix. What
stays intentionally unfixed: each `lyric-*/lyric.toml`'s committed
`version = "0.1.0"` line is never bumped by a release (a deliberate
choice — bumping and committing version bumps back to `main` from an
automated release run is a separate, heavier decision with its own
concurrency/timing concerns, not a prerequisite for correctly-versioned
release artifacts).
