...
| ~~`HttpClientHost`~~, `HttpServerHost` | 200 (HttpServerHost only) | G12 audit | `HttpClientHost.fs` deleted in PR #3016 (#1576 follow-up): replaced by `pub val defaultClient: HttpClient = newClient()` in `_kernel/http_host.l` (singleton via `.cctor`) plus direct BCL `@externTarget` declarations. `hostDefaultClient()` still calls `newClient()` per-call due to F# emitter `EPath` gap (tracked #3027). `HttpServerHost`: direct `@externTarget` against `System.Net.HttpListener.…` once delegate handling is fully audited. |
