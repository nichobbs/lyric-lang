...
15. `lyric_tls_*` OpenSSL seam in `lyric-rt` + socket transport + client;
    ...
    **N9.1 — the `lyric_sock_*` + `lyric_tls_*` seam — shipped**. It is a
    ...
    `hostShutdown`-ing each one before joining its handler thread — the
    #6792 regression — `spawnHandler` must register a connection
    ...
    thread), per D-progress-885's addendum in `docs/03-decision-log.md`.
    ...
    **N9.8 (D-progress-877, #6811)** ✅ SHIPPED: the `Std.Char` native
