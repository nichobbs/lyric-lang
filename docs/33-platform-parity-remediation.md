...
3. **Self-hosted emitter disconnect** — neither the self-hosted JVM emitter
   (`lyric/jvm/`) nor the self-hosted MSIL emitter
   (`lyric/msil/`) is reachable from `lyric build`.  The JVM emitter
...
Multiple entries reference `stdlib/std/core_proof.l`.  The correct path
...
**Status: SHIPPED** — `Jvm.Codegen` (`lyric/jvm/codegen.l`),
...
- `lyric/jvm/lowering.l` — complete high-level lowering (29 functions)
- `lyric/jvm/driver.l` — `writeJarFromClasses` JAR assembler
- `lyric/jvm/bytecode.l`, `classfile.l` — binary class-file emission
- `lyric/jvm/self_test_b*.l` — 125 self-tests (B3–B125)
- `lyric/lyric/parser/` — self-hosted parser (`Lyric.Parser`)
- `lyric/lyric/type_checker/` — self-hosted type checker
  (`Lyric.TypeChecker`)
...
**Status: SHIPPED** — `Msil.Lowering` (`lyric/msil/lowering.l`),
...
- `lyric/msil/pe.l` — raw PE binary writer
- `lyric/msil/opcodes.l` — IL opcode encoding
- `lyric/msil/tables.l` — CLI metadata table helpers
- `lyric/msil/heaps.l` — `#Strings`, `#Blob`, `#GUID`, `#US` heap writers
- `lyric/msil/assembler.l` — `assemblePe` — top-level assembler taking
  pre-built bodies and metadata rows

### 6.2 What is needed: `Msil.Lowering`

Location: `lyric/msil/lowering.l`
