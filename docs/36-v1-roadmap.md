...
| **R7.2 Backend correctness** | CRITICAL | **Yes** | `?` no-op (C3), compound-assign operator (H22), and Float/Long literal match compares (H18) are **fixed** (#1475, #1481 items 1–2). Remaining: `defer` runs inline not at scope exit (C7); `==` doesn't dispatch the derived `equals` (H1); capturing closures unimplemented (H20); `SItem`/`SInvariant` dropped (M7); #1481 items 3–4 (`break`/`continue` across `try` must `leave`; `List.Contains`/`removeAt` stubs). H17 (`break`/`continue` out of `try`) needs a targeted test. **The honest interim for any not-yet-lowered node is a hard diagnostic, never a silent pass-through.** |
