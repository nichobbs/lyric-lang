...
| C13 | **PARTIAL (2026-06-04).** Codegen passes `out`/`inout` by managed pointer (`isByrefMode`/`byrefWrapSigTypes`, #1761/D-progress-366). Front-end §5.2 enforcement (#1487): the **`in` no-rebind** rule is now enforced by the type checker for all packages (**T0087** — reassigning an `in` param or a `val`/`let`; resolves the V0001 numbering collision). Still OPEN: `out` definite-assignment (needs a definite-assignment pass) and `out`/`inout` argument-must-be-a-mutable-l-value at call sites (needs param modes threaded into call-site checking). (JVM by-ref parity is follow-up #1763.) | T0087 present; `out`/`inout` rules absent | M |
