...
| C8 | **PARTIAL — records fixed (#2362 slice 1 / D-progress-453).** In-bundle generic *records* are now reified: GenericParam rows (table 0x2A), VAR-typed fields (`value: T` → `FIELD VAR(0)`), arity-suffixed CLR type names (`Box`1`), AutoLayout, construction via a closed-instantiation TypeSpec `.ctor` MemberRef, and TypeSpec-parented field reads — verified for multi-field value-type instantiations (`Pair[Int,Int]`). Remaining: in-bundle generic *unions* (case classes, nullary-case singletons — Q-GEN-001) and the stage-3 stdlib byte-match. Plan + corrections: `docs/43`. | `codegen.l` (`lowerRecordMsil`, `typeExprToMsilCtx`, `buildInBundleGenericCtorTok`, EMember); `lowering.l` (`lowerMRecord`, GenericParam emit); `tables.l` (GenericParam 0x2A) | Records ✅; unions L |
