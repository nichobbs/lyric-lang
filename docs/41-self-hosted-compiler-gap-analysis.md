...
| C10 | Opaque representation-hiding not enforced: fields readable and types constructable from outside the declaring package. **PARTIAL (#1485/#1483):** cross-package **construction** of an opaque type is now rejected (**T0100**), and a constructor call yields the constructed type with named-field validation (**T0101**); field reads on opaque values already resolve to `TyError`. Pending: cross-package **pattern-match** representation-hiding (needs the pattern-field checking that also unblocks `EMatch`-branch typing). | `typechecker_exprs.l` (`inferConstruction`), `typechecker_checker.l` | M |
...
| C13 | **PARTIAL (2026-06-04).** Codegen passes `out`/`inout` by managed pointer (`isByrefMode`/`byrefWrapSigTypes`, #1761/D-progress-366). Front-end §5.2 enforcement (#1487): **`in` no-rebind** (**T0087** — reassigning an `in` param or a `val`/`let`; resolves the V0001 collision) and **`out`/`inout` argument-must-be-a-mutable-l-value** (**T0085**, value-type by-ref params only — reference-type `inout` records are mutated in place, so they're exempt) are now enforced by the type checker for all packages. Still OPEN: `out` definite-assignment (needs a definite-assignment dataflow pass). (JVM by-ref parity is follow-up #1763.) | T0085/T0087 present; `out` definite-assignment absent | M |
