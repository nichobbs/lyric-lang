...
/// `Lyric.Msil.Bridge.dll`, mirroring `SelfHostedMsil.ensureLyricMsilBridgeAssembly`.
let private ensureLyricMsilBridgeAssembly () : string =
    let scratch =
        Path.Combine(Path.GetTempPath(),
                     sprintf "lyric-msil-project-bridge-%d"
                         (System.Diagnostics.Process.GetCurrentProcess().Id))
    Directory.CreateDirectory scratch |> ignore
    AppDomain.CurrentDomain.ProcessExit.Add(fun _ ->
        try Directory.Delete(scratch, recursive = true) with _ -> ())
    let dllPath = Path.Combine(scratch, "Lyric.Msil.MsilProjectBridge.dll")
    let req : Emitter.EmitRequest =
        { Source             = driverSource
          AssemblyName       = "Lyric.Msil.MsilProjectBridge"
          OutputPath         = dllPath
          RestoredPackages   = []
          NugetAssemblyPaths = []
          ExternShimRoot     = None
    ...
    if not (List.isEmpty errs) then
        let msg =
            ...
            |> List.map (fun d -> sprintf "  %s @ %d:%d  %s"
                                       ...
                                       d.Message)
            |> String.concat "\n"
        failwithf "self-hosted MSIL project bridge: emitter produced errors:\n%s" msg
    ...
    | Some p -> p
    | None ->
        failwithf "self-hosted MSIL project bridge: 'Lyric.Msil.Bridge.dll' not found in stdlib cache"
