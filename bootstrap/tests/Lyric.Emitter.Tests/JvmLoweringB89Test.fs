/// Stage B89 smoke test — float[] arrays (newarray T_FLOAT, faload, fastore).
module Lyric.Emitter.Tests.JvmLoweringB89Test
...
let private findSelfTestB89Source () : string option =
    let mutable dir : DirectoryInfo option = Some (DirectoryInfo(AppContext.BaseDirectory))
    let mutable found : string option = None
    while found.IsNone && dir.IsSome do
        let candidate = Path.Combine(dir.Value.FullName, "lyric-compiler", "jvm", "self_test_b89.l")
...
let tests =
    testList "Jvm.Driver B89 (float[] arrays)" [

        testCase "b89_float_arrays" <| fun () ->
            let src =
                match findSelfTestB89Source () with
                | Some path -> File.ReadAllText path
                | None      -> failwith "cannot locate lyric-compiler/jvm/self_test_b89.l"

            let result, stdout, stderr, exitCode = compileAndRun "jvm_driver_b89" src
            ...
            let jarPath = "/tmp/lyric-jvm-b89/hello.jar"
