/// Phase 5 §M5.3 — exercises the self-hosted `Lyric.Manifest` TOML
/// parser through a tiny consumer.
///
/// Compiles `lyric-compiler/lyric/manifest_self_test.l` via the
/// bootstrap emitter, runs the resulting program, and asserts that
/// every in-program assertion held (exit code 0 + an "ok" line in
/// stdout).  The self-test imports `Lyric.Manifest`; the emitter's
/// auto-resolver pulls all multi-file libraries from
/// `lyric-compiler/lyric/` transparently.
module Lyric.Emitter.Tests.SelfHostedManifestTests
...
let private findSelfTestSource () : string option =
    ...
    while found.IsNone && dir.IsSome do
        let candidate =
            Path.Combine(dir.Value.FullName, "lyric-compiler", "lyric", "manifest_self_test.l")
        if File.Exists candidate then found <- Some candidate
        dir <- dir.Value.Parent |> Option.ofObj
    found

let tests =
    testList "Lyric.Manifest self-host (Phase 5 §M5.3)" [
        // DEFERRED: Test pending #2580 (in-process bridge compiler-package resolution).
        // The manifest_self_test.l file imports Lyric.Manifest (a compiler package),
        ...
        ptestCase "[manifest_self_test_passes (DEFERRED #2580)]" <| fun () ->
            let src =
                match findSelfTestSource () with
                | Some path -> File.ReadAllText path
                | None ->
                    failwith
                        "cannot locate lyric-compiler/lyric/manifest_self_test.l — run from the source tree"

            let result, stdout, stderr, exitCode =
                compileAndRun "self_hosted_manifest" src
