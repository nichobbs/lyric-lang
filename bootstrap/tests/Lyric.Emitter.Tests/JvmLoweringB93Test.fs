/// Stage B93 smoke test — lowerProtected (ReentrantLock-guarded counter).
module Lyric.Emitter.Tests.JvmLoweringB93Test
...
let private findSource () : string option =
    ...
    while found.IsNone && dir.IsSome do
        let candidate = Path.Combine(dir.Value.FullName, "lyric-compiler", "jvm", "self_test_b93.l")
...
let tests =
    testList "Jvm.Lowering B93 (lowerProtected)" [

        testCase "b93_lowerProtected" <| fun () ->
            let src =
                match findSource () with
                | Some path -> File.ReadAllText path
                | None      -> failwith "cannot locate lyric-compiler/jvm/self_test_b93.l"

            let result, stdout, stderr, exitCode = compileAndRun "jvm_driver_b93" src
            ...
            let jarPath = "/tmp/lyric-jvm-b93/hello.jar"
            ...
            Expect.equal lines.[0] "count=3"
                (sprintf "expected 'count=3', got '%s'" lines.[0])
    ]
