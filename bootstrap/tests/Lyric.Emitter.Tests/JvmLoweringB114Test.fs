/// Stage B114 smoke test — lowerNatTag (nat-tag codegen).
module Lyric.Emitter.Tests.JvmLoweringB114Test
...
let private findSource () : string option =
    ...
    while found.IsNone && dir.IsSome do
        let candidate = Path.Combine(dir.Value.FullName, "lyric-compiler", "jvm", "self_test_b114.l")
...
let tests =
    testList "Jvm.Lowering B114 (lowerNatTag)" [

        testCase "b114_nat_tag" <| fun () ->
            let src =
                match findSource () with
                | Some path -> File.ReadAllText path
                | None      -> failwith "cannot locate lyric-compiler/jvm/self_test_b114.l"

            let result, stdout, stderr, exitCode = compileAndRun "jvm_driver_b114" src
            ...
            let jarPath = "/tmp/lyric-jvm-b114/hello.jar"
            ...
            Expect.equal lines.[0] "n=42"   (sprintf "line 0: '%s'" lines.[0])
            Expect.equal lines.[1] "n=100"  (sprintf "line 1: '%s'" lines.[1])
    ]
