/// Stage B116 smoke test — lowerExposedRecord (exposed-record codegen).
module Lyric.Emitter.Tests.JvmLoweringB116Test
...
let private findSource () : string option =
    ...
    while found.IsNone && dir.IsSome do
        let candidate = Path.Combine(dir.Value.FullName, "lyric-compiler", "jvm", "self_test_b116.l")
...
let tests =
    testList "Jvm.Lowering B116 (lowerExposedRecord)" [

        testCase "b116_exposed_record" <| fun () ->
            let src =
                match findSource () with
                | Some path -> File.ReadAllText path
                | None      -> failwith "cannot locate lyric-compiler/jvm/self_test_b116.l"

            let result, stdout, stderr, exitCode = compileAndRun "jvm_driver_b116" src
            ...
            let jarPath = "/tmp/lyric-jvm-b116/hello.jar"
            ...
            Expect.equal lines.[0] "street=Main St"     (sprintf "line 0: '%s'" lines.[0])
            Expect.equal lines.[1] "city=Springfield"   (sprintf "line 1: '%s'" lines.[1])
    ]
