/// Stage B106 smoke test — lowerClosure (closure / inner-class codegen).
module Lyric.Emitter.Tests.JvmLoweringB106Test
...
let private findSource () : string option =
    ...
    while found.IsNone && dir.IsSome do
        let candidate = Path.Combine(dir.Value.FullName, "lyric-compiler", "jvm", "self_test_b106.l")
...
let tests =
    testList "Jvm.Lowering B106 (lowerClosure)" [

        testCase "b106_closure_captures" <| fun () ->
            let src =
                match findSource () with
                | Some path -> File.ReadAllText path
                | None      -> failwith "cannot locate lyric-compiler/jvm/self_test_b106.l"

            let result, stdout, stderr, exitCode = compileAndRun "jvm_driver_b106" src
            ...
            let jarPath = "/tmp/lyric-jvm-b106/hello.jar"
            ...
            Expect.equal lines.[0] "result(3+4+10)=17" (sprintf "line 0: '%s'" lines.[0])
            Expect.equal lines.[1] "result(5+6+0)=11"  (sprintf "line 1: '%s'" lines.[1])
    ]
