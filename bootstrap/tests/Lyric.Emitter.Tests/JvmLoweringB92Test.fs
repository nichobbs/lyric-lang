/// Stage B92 smoke test — lowerUnion.
module Lyric.Emitter.Tests.JvmLoweringB92Test
...
let private findSource () : string option =
    ...
    while found.IsNone && dir.IsSome do
        let candidate = Path.Combine(dir.Value.FullName, "lyric-compiler", "jvm", "self_test_b92.l")
...
let tests =
    testList "Jvm.Lowering B92 (lowerUnion)" [

        testCase "b92_lowerUnion" <| fun () ->
            let src =
                match findSource () with
                | Some path -> File.ReadAllText path
                | None      -> failwith "cannot locate lyric-compiler/jvm/self_test_b92.l"

            let result, stdout, stderr, exitCode = compileAndRun "jvm_driver_b92" src
            ...
            let jarPath = "/tmp/lyric-jvm-b92/hello.jar"
            ...
            Expect.equal lines.[0] "circle.radius=5"
                (sprintf "expected 'circle.radius=5', got '%s'" lines.[0])

            Expect.equal lines.[1] "square.side=8"
                (sprintf "expected 'square.side=8', got '%s'" lines.[1])
    ]
