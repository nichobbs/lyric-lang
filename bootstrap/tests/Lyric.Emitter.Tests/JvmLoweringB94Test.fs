/// Stage B94 smoke test — lowerWire (static bootstrap + accessor).
module Lyric.Emitter.Tests.JvmLoweringB94Test
...
let private findSource () : string option =
    ...
    while found.IsNone && dir.IsSome do
        let candidate = Path.Combine(dir.Value.FullName, "lyric-compiler", "jvm", "self_test_b94.l")
...
let tests =
    testList "Jvm.Lowering B94 (lowerWire)" [

        testCase "b94_lowerWire" <| fun () ->
            let src =
                match findSource () with
                | Some path -> File.ReadAllText path
                | None      -> failwith "cannot locate lyric-compiler/jvm/self_test_b94.l"

            let result, stdout, stderr, exitCode = compileAndRun "jvm_driver_b94" src
            ...
            let jarPath = "/tmp/lyric-jvm-b94/hello.jar"
            ...
            Expect.equal lines.[0] "answer=42"
                (sprintf "expected 'answer=42', got '%s'" lines.[0])
    ]
