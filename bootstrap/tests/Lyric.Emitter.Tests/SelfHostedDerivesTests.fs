/// Phase 5 §M5.2 Band 5 (#857) — exercises the self-hosted `Lyric.Derives`
/// library through a tiny Lyric.DerivesSelfTest consumer.
///
/// Compiles `lyric-compiler/lyric/derives_self_test.l` via the bootstrap
/// emitter, runs the resulting program, and asserts that every
/// in-program assertion held (exit code 0 + an "ok" line in stdout).
/// The self-test imports `Lyric.Derives` (and transitively `Lyric.Parser`);
...
module Lyric.Emitter.Tests.SelfHostedDerivesTests
...
let private findSelfTestSource () : string option =
    ...
    while found.IsNone && dir.IsSome do
        let candidate =
            Path.Combine(dir.Value.FullName, "lyric-compiler", "lyric", "derives_self_test.l")
        if File.Exists candidate then found <- Some candidate
        dir <- dir.Value.Parent |> Option.ofObj
    found

let tests =
    testList "Lyric.Derives self-host (Phase 5 §M5.2 Band 5 #857)" [

        testCase "[derives_self_test_passes]" <| fun () ->
            let src =
                match findSelfTestSource () with
                | Some path -> File.ReadAllText path
                | None ->
                    failwith
                        "cannot locate lyric-compiler/lyric/derives_self_test.l — run from the source tree"

            let result, stdout, stderr, exitCode =
                compileAndRun "self_hosted_derives" src
