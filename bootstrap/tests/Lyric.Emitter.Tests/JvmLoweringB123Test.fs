/// Stage B123 smoke test — lowerDeriveOrd (derive-ord codegen).
module Lyric.Emitter.Tests.JvmLoweringB123Test
...
let private findSource () : string option =
    ...
    while found.IsNone && dir.IsSome do
        let candidate = Path.Combine(dir.Value.FullName, "lyric-compiler", "jvm", "self_test_b123.l")
...
let tests =
    testList "Jvm.Lowering B123 (lowerDeriveOrd)" [

        testCase "b123_derive_ord" <| fun () ->
            let src =
                match findSource () with
                | Some path -> File.ReadAllText path
                | None      -> failwith "cannot locate lyric-compiler/jvm/self_test_b123.l"

            let result, stdout, stderr, exitCode = compileAndRun "jvm_driver_b123" src
            ...
            let jarPath = "/tmp/lyric-jvm-b123/hello.jar"
            ...
            Expect.equal lines.[0] "cmp=-1"   (sprintf "line 0: '%s'" lines.[0])
            Expect.equal lines.[1] "sum=130"  (sprintf "line 1: '%s'" lines.[1])
    ]
