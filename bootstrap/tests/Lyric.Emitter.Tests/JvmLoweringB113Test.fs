/// Stage B113 smoke test — lowerOutInoutParam (out/inout parameter codegen).
module Lyric.Emitter.Tests.JvmLoweringB113Test
...
let private findSource () : string option =
    ...
    while found.IsNone && dir.IsSome do
        let candidate = Path.Combine(dir.Value.FullName, "lyric-compiler", "jvm", "self_test_b113.l")
...
let tests =
    testList "Jvm.Lowering B113 (lowerOutInoutParam)" [

        testCase "b113_out_param" <| fun () ->
            let src =
                match findSource () with
                | Some path -> File.ReadAllText path
                | None      -> failwith "cannot locate lyric-compiler/jvm/self_test_b113.l"

            let result, stdout, stderr, exitCode = compileAndRun "jvm_driver_b113" src
            ...
            let jarPath = "/tmp/lyric-jvm-b113/hello.jar"
            ...
            Expect.equal lines.[0] "result=6"  (sprintf "line 0: '%s'" lines.[0])
    ]
