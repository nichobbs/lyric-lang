/// Stage B118 smoke test — lowerProtectedWithBarriers (protected-type barrier codegen).
module Lyric.Emitter.Tests.JvmLoweringB118Test
...
let private findSource () : string option =
    ...
    while found.IsNone && dir.IsSome do
        let candidate = Path.Combine(dir.Value.FullName, "lyric-compiler", "jvm", "self_test_b118.l")
...
let tests =
    testList "Jvm.Lowering B118 (lowerProtectedWithBarriers)" [

        testCase "b118_protected_barriers" <| fun () ->
            let src =
                match findSource () with
                | Some path -> File.ReadAllText path
                | None      -> failwith "cannot locate lyric-compiler/jvm/self_test_b118.l"

            let result, stdout, stderr, exitCode = compileAndRun "jvm_driver_b118" src
            ...
            let jarPath = "/tmp/lyric-jvm-b118/hello.jar"
            ...
            Expect.equal lines.[0] "result=42"  (sprintf "line 0: '%s'" lines.[0])
    ]
