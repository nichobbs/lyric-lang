...
let prepareOutputDir (name: string) : string =
    ...
    // Lyric.Emitter.dll is still needed at runtime by the kernel that has
    // not yet migrated off it: Std.ProcessCaptureHost (ProcessCapture).
    // Std.HttpHost's defaultClient migrated to a native Lyric val (#1576).
    // The console/env/log kernels no longer extern into it (#1493).
    let lyricEmitter =
