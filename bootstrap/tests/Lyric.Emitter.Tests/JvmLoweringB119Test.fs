/// Stage B119 smoke test — lowerHotAsync (hot-async codegen).
module Lyric.Emitter.Tests.JvmLoweringB119Test
...
let private findSource () : string option =
    ...
    while found.IsNone && dir.IsSome do
        let candidate = Path.Combine(dir.Value.FullName, "lyric-compiler", "jvm", "self_test_b119.l")
...
let tests =
    testList "Jvm.Lowering B119 (lowerHotAsync)" [

        testCase "b119_hot_async" <| fun () ->
            let src =
                match findSource () with
                | Some path -> File.ReadAllText path
                | None      -> failwith "cannot locate lyric-compiler/jvm/self_test_b119.l"

            let result, stdout, stderr, exitCode = compileAndRun "jvm_driver_b119" src
            ...
            let jarPath = "/tmp/lyric-jvm-b119/hello.jar"
            ...
            Expect.equal lines.[0] "result=2"  (sprintf "line 0: '%s'" lines.[0])
    ]
