/// Stage B127 smoke test — Std.Jvm.catch intrinsic try-catch bytecode pattern.
module Lyric.Emitter.Tests.JvmLoweringB127Test
...
let private findSource () : string option =
    ...
    while found.IsNone && dir.IsSome do
        let candidate = Path.Combine(dir.Value.FullName, "lyric-compiler", "jvm", "self_test_b127.l")
...
let tests =
    testList "Jvm.Lowering B127 (catch intrinsic try-catch pattern)" [

        testCase "b127_catch_intrinsic_ok_and_err" <| fun () ->
            let src =
                match findSource () with
                | Some path -> File.ReadAllText path
                | None      -> failwith "cannot locate lyric-compiler/jvm/self_test_b127.l"

            let result, stdout, stderr, exitCode = compileAndRun "jvm_driver_b127" src
            ...
            let jarPath = "/tmp/lyric-jvm-b127/catch.jar"
            ...
            Expect.equal lines.[0] "divide(10,2)=ok:5"
                (sprintf "line 0 (ok path): '%s'" lines.[0])
            Expect.equal lines.[1] "divide(10,0)=err:ArithmeticException"
