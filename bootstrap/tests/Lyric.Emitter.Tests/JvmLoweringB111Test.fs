/// Stage B111 smoke test — lowerSealedUnion (sealed-union codegen).
module Lyric.Emitter.Tests.JvmLoweringB111Test
...
let private findSource () : string option =
    ...
    while found.IsNone && dir.IsSome do
        let candidate = Path.Combine(dir.Value.FullName, "lyric-compiler", "jvm", "self_test_b111.l")
...
let tests =
    testList "Jvm.Lowering B111 (lowerSealedUnion)" [

        testCase "b111_sealed_union" <| fun () ->
            let src =
                match findSource () with
                | Some path -> File.ReadAllText path
                | None      -> failwith "cannot locate lyric-compiler/jvm/self_test_b111.l"

            let result, stdout, stderr, exitCode = compileAndRun "jvm_driver_b111" src
            ...
            let jarPath = "/tmp/lyric-jvm-b111/hello.jar"
            ...
            Expect.equal lines.[0] "circle.radius=1.5"  (sprintf "line 0: '%s'" lines.[0])
            Expect.equal lines.[1] "rect.area=12.0"     (sprintf "line 1: '%s'" lines.[1])
    ]
