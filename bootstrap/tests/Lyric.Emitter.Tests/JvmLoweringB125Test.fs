/// Stage B125 smoke test — lowerOpaqueFacade (Java interop facade codegen).
module Lyric.Emitter.Tests.JvmLoweringB125Test
...
let private findSource () : string option =
    ...
    while found.IsNone && dir.IsSome do
        let candidate = Path.Combine(dir.Value.FullName, "lyric-compiler", "jvm", "self_test_b125.l")
...
let tests =
    testList "Jvm.Lowering B125 (lowerOpaqueFacade)" [

        testCase "b125_lower_opaque_facade" <| fun () ->
            let src =
                match findSource () with
                | Some path -> File.ReadAllText path
                | None      -> failwith "cannot locate lyric-compiler/jvm/self_test_b125.l"

            let result, stdout, stderr, exitCode = compileAndRun "jvm_driver_b125" src
            ...
            let jarPath = "/tmp/lyric-jvm-b125/hello.jar"
            ...
            Expect.equal lines.[0] "x=10" (sprintf "line 0: '%s'" lines.[0])
            Expect.equal lines.[1] "y=20" (sprintf "line 1: '%s'" lines.[1])
    ]
