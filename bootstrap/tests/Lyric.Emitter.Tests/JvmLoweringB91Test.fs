/// Stage B91 smoke test — lowerRecord.
module Lyric.Emitter.Tests.JvmLoweringB91Test
...
let private findSource () : string option =
    ...
    while found.IsNone && dir.IsSome do
        let candidate = Path.Combine(dir.Value.FullName, "lyric-compiler", "jvm", "self_test_b91.l")
...
let tests =
    testList "Jvm.Lowering B91 (lowerRecord)" [

        testCase "b91_lowerRecord" <| fun () ->
            let src =
                match findSource () with
                | Some path -> File.ReadAllText path
                | None      -> failwith "cannot locate lyric-compiler/jvm/self_test_b91.l"

            let result, stdout, stderr, exitCode = compileAndRun "jvm_driver_b91" src
            ...
            let jarPath = "/tmp/lyric-jvm-b91/hello.jar"
            ...
            Expect.equal lines.[0] "point.x=3"
                (sprintf "expected 'point.x=3', got '%s'" lines.[0])

            Expect.equal lines.[1] "point.y=7"
                (sprintf "expected 'point.y=7', got '%s'" lines.[1])
    ]
