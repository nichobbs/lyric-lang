/// Stage B115 smoke test — makeLyricSignatureAttr (Lyric signature attribute codegen).
module Lyric.Emitter.Tests.JvmLoweringB115Test
...
let private findSource () : string option =
    ...
    while found.IsNone && dir.IsSome do
        let candidate = Path.Combine(dir.Value.FullName, "lyric-compiler", "jvm", "self_test_b115.l")
...
let tests =
    testList "Jvm.Lowering B115 (makeLyricSignatureAttr)" [

        testCase "b115_lyric_signature_attr" <| fun () ->
            let src =
                match findSource () with
                | Some path -> File.ReadAllText path
                | None      -> failwith "cannot locate lyric-compiler/jvm/self_test_b115.l"

            let result, stdout, stderr, exitCode = compileAndRun "jvm_driver_b115" src
            ...
            let jarPath = "/tmp/lyric-jvm-b115/hello.jar"
            ...
            Expect.equal lines.[0] "sig_class_valid=true"  (sprintf "line 0: '%s'" lines.[0])
    ]
