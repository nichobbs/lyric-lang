/// Stage B103 smoke test — maxStack/maxLocals inference via trackStack.
module Lyric.Emitter.Tests.JvmLoweringB103Test
...
let private findSource () : string option =
    ...
    while found.IsNone && dir.IsSome do
        let candidate = Path.Combine(dir.Value.FullName, "lyric-compiler", "jvm", "self_test_b103.l")
...
let tests =
    testList "Jvm.Lowering B103 (maxStack/maxLocals inference)" [

        testCase "b103_inferred_stack" <| fun () ->
            let src =
                match findSource () with
                | Some path -> File.ReadAllText path
                | None      -> failwith "cannot locate lyric-compiler/jvm/self_test_b103.l"

            let result, stdout, stderr, exitCode = compileAndRun "jvm_driver_b103" src
            ...
            let jarPath = "/tmp/lyric-jvm-b103/hello.jar"
