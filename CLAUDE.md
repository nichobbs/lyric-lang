...
- `docs/62-jsonrpc-mcp.md` — JSON-RPC 2.0 and Model Context Protocol libraries: `lyric-jsonrpc` (strict cross-target `JsonRpc.Json` value model, symmetric `RpcPeer` over an `RpcTransport` seam, NDJSON + Content-Length stdio framings) and `lyric-mcp` (MCP 2025-06-18 client + server, stdio transport first, streamable HTTP on lyric-web SSE second), plus the lyric-ws dotnet backend completion (#778, pure-Lyric RFC 6455 over `Std.TcpHost`) and the `Std.Hash.sha1OfBytes` / `Std.Process` piped-spawn stdlib seams they need. First consumer: cloud-agents' in-container permission-callback MCP server. _Specced in D129; jsonrpc + mcp-stdio + ws dotnet backend implemented (JVM gaps tracked in #6118–#6124, #6127, #6133–#6136); streamable HTTP milestone 2 open, plus Q-RPC-001/002, Q-MCP-001–003, Q-WS-001._
