...
- `docs/51-ffi-interfaces-proposal.md` — Implementing external .NET interfaces from Lyric: an `impl <ExternInterface> for Record { … }` block emits an `InterfaceImpl` row against the existing TypeRef, with the CLR FQN resolved through the `import extern` / `extern type` table. _Specced in D105. Phases 1 (non-generic emission) and 2 (metadata-based signature validation, `F0020`–`F0023`) shipped; generic interfaces and bridge-thunk synthesis deferred._
