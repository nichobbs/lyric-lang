...
- `docs/50-ffi-delegates-proposal.md` — Proposal to pass Lyric lambdas/method references to .NET methods expecting strongly-typed delegates via auto-FFI: once lambdas are strongly typed (Epic #1877), a Lyric `TFunction` signature structurally matches a .NET delegate's `.Invoke` signature, so FFI bridging collapses to a direct `ldftn`/`newobj` delegate instantiation with no adapter-thunk synthesis. Depends on `docs/52`/`docs/53`. _Specced in D122: shipped bounded to `@externTarget` functions' own `TFunction`-typed parameters (not the general "every lambda gets a nominally-typed delegate" ABI this doc originally proposed — that would need to touch `typeExprToMsilCtx`'s shared default, too much blast radius). Verified against a real BCL API with a value-type delegate argument (`SocketsHttpHandler.ConnectCallback`). `UnixSocketHttpClient.cs` migrated to pure Lyric in D-progress-686 (`Std.HttpHost`), the last CLI-host-assembly dependency in the HTTP stack — closed three latent `emitGenericExternMember` bugs constructing/returning a value-type generic extern (`ValueTask`1`); see D-progress-686 for the scope boundary (instance-method calls on a value-type generic receiver remain unsupported, separate follow-up)._
- `docs/51-ffi-interfaces-proposal.md` — Implementing external .NET interfaces from Lyric: an `impl <ExternInterface> for Record { … }` block emits an `InterfaceImpl` row against the existing TypeRef, with the CLR FQN resolved through the `import extern` / `extern type` table. _Specced in D105. Phases 1 (non-generic emission), 2 (metadata-based signature validation, `F0021`–`F0023`, `F0034`), and 3 (widening to extern-interface-typed bindings / parameters) shipped; closed generic external interfaces (`IEquatable<T>`, `IComparable<T>`, `IEnumerable<T>`-shaped) ship via TypeSpec emission + CLR name-matching through the TypeSpec; structural validation substitutes `STVar` against the iface's resolved type args and lifts F0024 for `STSzArray` / `STByRef` / `STNamedGenericInst` shapes, leaving `F0024` narrowed to (never actually removed from) its one remaining job: `validateOneExternImplMsil`'s iface-FQN-typo guard, live and firing continuously since before #6547 — an earlier review pass on #6547 mistook the panic→diagnostic conversion (#4898) for a brand-new use of the code; see `docs/51`'s own "F0024" section for the correction. (`F0020` is unrelated to this family — it is `propagate.l`'s pre-existing "`?` in a non-`Result`/`Option` function" check; the FFI "not an interface" diagnostic that once collided with it was renumbered to `F0034` by #6648.)  `STMVar` (method-generic iface methods) and exotic shapes (`STPtr`, `STFnPtr`, `STArray` rank > 1) silent-skip per-method F0022/F0023 validation; F0021 still fires for any missing impl methods on the iface. Bridge-thunk synthesis is confirmed N/A for the current Lyric MSIL ABI; LSP "Implement Interface" scaffolding shipped in #3861. Remaining-work implementation plans live in docs/51 §"Remaining work" with concrete file/line references._
...
- `docs/57-stdlib-ecosystem-library-review.md` — audit (2026-07) of `lyric-stdlib/` and all 26 ecosystem libraries for FFI-idiom currency (`import extern`/`.new()` adoption), MSIL/JVM/native/local backend parity, aspect-idiom currency, and test coverage. Headline finding: type-checker support for `import extern` + `.new()` landed in commit `a64e649` (2026-07-03) but `docs/47`/`docs/48` status headers still say "deferred," so zero stdlib or ecosystem code has adopted it yet and `import_extern_self_test.l` isn't wired into CI. Catalogues a constructor-wrapper → `.new()` migration backlog, JVM/backend stub gaps (lyric-resilience JVM kernel, lyric-storage S3/Azure, lyric-mq/lyric-jobs broker backends, lyric-search), a stale `lyric-cache/src/cache_aspects.l` header claiming the aspect weaver is unshipped, stdlib core-I/O test gaps (`file.l`/`directory.l`/`http.l` have no dedicated test file), and missing `Lyric.Testing` mocks (Db/Jobs/Search/i18n/Ws). _Unbacked — see §7 for suggested rollout order; no decision-log entry yet._
- `docs/58-wire-templates-sketch.md` — proposes a library-to-consumer DI extension mechanism motivated by `lyric-web` needing typed, overridable config for static files, error handlers, and an ordered middleware pipeline: **config templates** (`pub config` + `from`, relaxing `G0008` for a template form), **`contributes[T]`** ordered multi-value wire bindings (Dagger-multibinding-shaped, not Spring's reflective collection autowiring), and **wire templates** (`pub wire` + `include` — no separate `wire template` keyword, per D051's "one keyword, different modifiers" precedent already applied to aspects — modeled as "a parameterized function from `@provided` inputs to `expose`d outputs, spliced textually — never a runtime container"). Generalizes the aspect-template (`docs/26` §18) "library declares, consumer instantiates with local overrides" idiom beyond `around` advice. Concludes wire templates are **precompiled only** — no C-mode/source-splice analog — because every extension point is nominally typed by the module author up front, unlike aspects' `matches:`-induced opaque `args` shape; `@cfg` inside a template is therefore never a consumer-facing axis, only runtime config toggles are. _Specced and implemented in D121: `Lyric.WireExpand` (`lyric-compiler/lyric/wire_expand/`) expands all three pieces before type checking on both backends (Q-wire-001 → open + `sealed` opt-in; Q-wire-002 → scope propagation, no auto-re-expose); the MSIL wire bootstrap/accessor ABI (#5021) was fixed alongside so wires are callable on both targets. Verified by `wire_expand_self_test.l` (expander unit), `wire_templates_self_test.l` (runtime, both targets), `config_templates_self_test.l` (runtime, dotnet — the docs/58 config-template/wire-embedded layering's JVM lowering is a separate follow-up, tracked in #6213; base module-scope `config { }` blocks now emit on JVM via #3228, covered by `config_block_self_test.l` on both targets)._
- `docs/59-compiler-stdlib-deep-review.md` — deep audit (2026-07-10) of the self-hosted compiler and standard library: silent-miscompile findings on both backends (MSIL restored-interface signature-encoder mismatch; JVM match-arm pattern-bind shadowing, compound-assignment `String`/`Float` holes, dual `Byte` representations), type-checker `TyError` degradation and monomorphizer gaps, an FFI capability matrix quantifying what blocks retiring `@externTarget` (property access ~24% of kernel externs, value-type receivers, async unwrap, out/ref; ~54% migratable today but seed-blocked by #5167), stdlib kernel three-tree parity breaks (five phantom `_kernel_jvm` classes leave `Std.Http`/`Std.HttpServer`/`Std.Path`/`Std.Format` non-functional on JVM), public-API consistency/consolidation findings (incl. a `Std.Log` structured-fields panic), MSIL↔JVM duplication with demonstrated one-sided-fix divergence, ~40k lines of orphaned numbered backend self-tests, and a banded remediation plan. _Unbacked — audit; see §10 for the remediation order; no decision-log entry yet._
- `docs/60-build-defines.md` — compile-time *value-injection* mechanism (the substitution counterpart to `docs/24`'s `@cfg` erasure): a `@build_const("K")` annotation on a module-level `String` `val` whose initializer literal doubles as a dev-checkout fallback, fed from a `[build.define]` manifest table + repeatable `--define K=V` CLI flag (CLI over manifest over auto-injected well-known `version`/`target`/`build_profile` defines), substituted into the AST as a `String`-literal node in the `pipeParseAndErase` stage beside `applyCfgErasure` — no backend codegen changes. Generalizes D127's runtime `LYRIC_BUILD_VERSION` env-var version workaround into a compile-time, self-hosted, AOT-trim-safe constant. Recommends the annotation form over an `env!`-style `buildValue(...)` intrinsic (no in-source fallback, new resolver/formatter surface). Resolves Q-BD-001–Q-BD-009 (String-only v1; build-private cross-package like features; reproducible-by-construction since the bootstrap byte-compare supplies no defines; no source-injection surface — values are literal AST nodes, never re-parsed; and a synthesized-into-root `Std.BuildInfo` layer that unifies docs/24 Q-features-003). _**M1a shipped** (PR #5869): the `@build_const` substitution pass (`Lyric.BuildDefines`) + `lyric build --define KEY=VALUE` for single-file `--target dotnet`, diagnostics F0030–F0032. Project-path/JVM/native/manifest/well-known-defines/`Std.BuildInfo` are follow-ups (#5852). Extends docs/24 (D045) Q-features-004; decision-log entry lands with the full design._
- `docs/61-https-tls-http-versions.md` — HTTPS/TLS support across client and server on all three targets: `Std.Tls` module (PEM-only cert/key loading), `HttpClientBuilder` TLS surface, dual-key insecure policy (`withInsecureSkipVerify()` + `LYRIC_TLS_ALLOW_INSECURE`), h2-or-lower client default on both targets, `TlsServerConfig` with v1 mTLS, JVM server TLS on `HttpsServer`/Undertow, a pure-Lyric sans-IO HTTP protocol engine (`Std.HttpEngine`) replacing the dotnet `HttpListener` (TLS via `SslStream` transport kernel; h2 implemented once in the engine, ALPN-only), and OpenSSL 3.x-dynamic as the native TLS seam. _Specced in D128; implementation tracked under epic #5874. Open questions Q-TLS-001–Q-TLS-006._
- `docs/62-jsonrpc-mcp.md` — JSON-RPC 2.0 and Model Context Protocol libraries: `lyric-jsonrpc` (strict cross-target `JsonRpc.Json` value model, symmetric `RpcPeer` over an `RpcTransport` seam, NDJSON + Content-Length stdio framings) and `lyric-mcp` (originally speced for MCP 2025-06-18, now superseded by docs/64's 2026-07-28 migration; stdio transport first, streamable HTTP on lyric-web SSE second), plus the lyric-ws dotnet backend completion (#778, pure-Lyric RFC 6455 over `Std.TcpHost`) and the `Std.Hash.sha1OfBytes` / `Std.Process` piped-spawn stdlib seams they need. First consumer: cloud-agents' in-container permission-callback MCP server. _Specced in D129; jsonrpc + mcp-stdio + ws dotnet backend implemented (JVM gaps tracked in #6118–#6124, #6127, #6133–#6136); streamable HTTP milestone 2 open (now docs/64 §4, Phase B), plus Q-RPC-001/002, Q-WS-001. Q-MCP-001 (sampling) superseded — 2026-07-28 deprecates the target outright; Q-MCP-002 (long-blocking tools) addressed by docs/64's `input_required` pattern; Q-MCP-003 (OAuth) stays open._
- `docs/63-build-profiles-and-debugger.md` — build **profile** (`--debug`/`--release`) and output **shape** (`--shape portable|standalone|aot`) as axes distinct from each other and from `--target`, plus the debug-information emission and debugger architecture the split unblocks. Documents that `--release` today selects a *packaging mode* (self-contained Native AOT via `Lyric.Release`), not an optimization/symbol profile — conflating optimize/strip/self-contain/AOT into one flag — and that **no target emits any debug information** (no PDB or PE Debug Directory on MSIL, `SourceFile` but no `LineNumberTable` on JVM, no `-g`/DWARF on native), while the front-end `Span` data that a debugger needs never reaches any backend IR and the middle end (weaver `synSpan()`, contract elaborator, `Lyric.Mono` renaming) actively destroys source fidelity. Settles three framing decisions: a **clean break** on `--release` (no deprecation window; `[build] kind = "aot"` becomes hard error `F0042`), profile and shape as **independent axes** (unlocking the currently-unreachable optimized-portable-DLL and debuggable-AOT combinations), and a **hybrid staged** debugger emitting both host formats and `LyricSourceMap` side tables from one span pass — thin host-debugger shim first, Lyric-aware DAP *proxy* later, never a JVMTI/ICorDebug agent. Bands B0–B7 in §9 (B0 flags alone; JVM debug info before MSIL/native, per §9.2). _Band B0 shipped and codified in D132 (profile/shape axes, `--shape` + sugar, `[build] shape`/`profile`, gates re-scoped onto `usesPackagingPipeline`, F0040–F0044, clean break on `kind = "aot"`). Band B1 slices 1–2 shipped in D-progress-804 (§9.7): diagnostics carry the real source path (single-file builds) or package label (project builds, parse-phase gap closed too); the JVM backend emits a real `SourceFile` attribute for single-file and single-package-project builds. B1 slice 3 (#6282's multi-file per-file-parse rewrite) and slices/bands B2–B7 remain open. Open questions Q-BP-001–Q-BP-011._
- `docs/64-mcp-2026-stateless-migration.md` — migration plan for `lyric-mcp`/`lyric-jsonrpc` from MCP spec revision `2025-06-18` (docs/62/D129) to `2026-07-28`, the largest protocol revision since MCP launched: stateless core (removes `initialize`/`initialized` and `Mcp-Session-Id`; `server/discover`), `input_required` multi-round-trip results (resolves docs/62's Q-MCP-002 for long-blocking permission-prompt tools — cloud-agents' actual motivating use case), streamable HTTP transport updated for the new `Mcp-Method`/`Mcp-Name` header contract and `ttlMs`/`cacheScope` cache metadata (the docs/62 §5.3 "milestone 2" that was never built), and a Tasks extension (poll-based `tasks/get`/`cancel`). Explicitly scopes OAuth/auth-hardening (RFC 9207, DCR, CIMD) **out** — those SEPs harden an existing OAuth 2.1 implementation, and `lyric-mcp` has never had one (Q-MCP-003 stays open). _Phase A (stateless core) specced and implemented in D133 (59/59 `--target dotnet`); Phases B (streamable HTTP) and C (Tasks) remain open. Per-request `_meta` version/capability negotiation was considered but not implemented — no known consumer needs it (deferred, #6344)._
- `docs/65-ui-library-sketch.md` — UI library design: one model-view-update application model for desktop and web (`lyric-ui`, `lyric-forms`). Screens are five enforced layers (domain / ports / logic / effects / view) with effects returned as data; a general `[layers]` language feature plus a stdlib `@pure`/`@io` classification enforces the separation; a closed set of semantic widgets renders through a small TypeScript host runtime (the one accepted non-Lyric UI component); hosts are server-driven web over `lyric-ws` first, desktop webview second, client WASM via the native backend later. Covers composition (router-owned screens, explicit nesting, dialogs as effects), the `View` diff/patch protocol with input versioning, and `@generate(Forms.Derive)` form derivation. _Specced in D137; phases U1 (pure core, `lyric-forms`, `examples/ui-customers/`) and U2 (server-driven web host + TypeScript runtime) implemented on MSIL (#7373); §15 records the compiler gaps the first implementation surfaced (cross-package generics, record `.copy`) and the fixes that resolved them (#7215, #7250, D-progress-970); open questions Q-UI-001–Q-UI-011 all resolved (Q-UI-001–010 in D138: keyed event paths, session resume/eviction, `FieldPath`/`DraftRows` implemented; `Ctx`, purity rules, generator schema v2, `raw` feature gate, JVM desktop binding and runtime schema generation recorded as designs). `lyric-ui` does not yet build on JVM (#7378)._
