...
- `docs/54-docker-client-library-sketch.md` — Design for `lyric-docker`, a type-safe Docker daemon API client over Unix sockets (Linux/macOS) / named pipes (Windows), with OpenAPI-generated bindings and `Result[T, Error]`-returning async operations. _Doc's own status header: "Phase 1 shipped (D-progress-TBD); Phase 2 planned" — the `D-progress-TBD` placeholder is stale and should be resolved to the actual `docs/10-bootstrap-progress.md` entry number in a follow-up._
- `docs/55-bmode-aspect-libraries-plan.md` — implementation plan for cross-package library aspect distribution's B-mode half (extends docs/27 §6.1, Q-aspectlib-001/-009). Documents that B-mode was speced (D047-revision 2026-05-08) but never implemented — only C-mode (`@inline_template`) exists — because it requires reified generic *methods* (MVAR), which the self-hosted MSIL backend doesn't support (only generic *types* are reified; docs/43 Q-GEN-002 leaves generic methods unresolved). Proposes **B′-mode**: a monomorphisation-based variant built on the existing `Lyric.Mono.monoFileWithImports` cross-package generic-function specialiser instead of reified generics, getting B-mode's zero-boxing/type-safety properties without the MVAR prerequisite (but not the "callable from non-Lyric consumers" property, which only true reified generics would provide). _Unbacked — no decision-log entry yet._
- `docs/56-row-typed-aspect-args-sketch.md` — pressure-test sketch for row-polymorphic (`where TArgs has {field: Type}`) named-field access on B′-mode aspect `args`, extending docs/55. Scopes the constraint-satisfaction check to compiler-synthesized `TArgs` only (never user-nameable), keeping it a bounded per-specialisation-site structural check rather than general row-type unification; compares against a narrower "auto-synthesized marker interface" alternative (reusing docs/51's `InterfaceImpl` emission) that gets the same functional result with zero new language surface. _Unbacked (Q-row-001–Q-row-005 open); cited from docs/27 §12 as Q-aspectlib-009._
