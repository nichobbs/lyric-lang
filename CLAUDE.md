...
- `docs/51-ffi-interfaces-proposal.md` — Implementing external .NET interfaces from Lyric: an `impl <ExternInterface> for Record { … }` block emits an `InterfaceImpl` row against the existing TypeRef, with the CLR FQN resolved through the `import extern` / `extern type` table. _Specced in D105. Phase 1 (non-generic emission) shipped; metadata-based signature validation (`F0020`–`F0023`), generic interfaces, the `get_`/`set_` property convention, and bridge-thunk synthesis deferred._
