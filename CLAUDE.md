...
- `docs/46-const-patterns.md` — const patterns in match arms: `@Ident` syntax to reference compile-time `val` constants instead of raw literals, disambiguated from variable bindings, zero runtime overhead via lowering to `PLiteral`. _Unbacked (Q-MP-001 open in docs/06)._
