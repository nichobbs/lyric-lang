...
`Web.OpenApi` is a hand-built spec vocabulary — populate a `Spec` value in your package, then either:

- Attach it to a running server with `Web.withSpec(router, spec)`, which serves the live OpenAPI 3.1 JSON document at `<pathPrefix>/openapi.json` and a Swagger UI page at `<pathPrefix>/swagger` (#5360). Both are ordinary routes registered through the normal pipeline, so existing middleware (auth, CORS, rate limiting, …) applies to them exactly like any other route.
- Render it to a file at build time with `Web.OpenApi.specToJson(spec)` (write the result with `Std.File.writeText`).
- Or run `lyric generate openapi spec.yaml --out src/generated/` to scaffold record types + handler stubs from an *existing* spec file (spec-first, the other direction).

See the module doc comment in `lyric-web/src/openapi.l` for the full type mapping and constraint-to-`requires:` table.
