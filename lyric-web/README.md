...
- **JVM target's test suite does not compile today**, for reasons entirely outside this library — two distinct, newly-discovered JVM backend bugs, neither of which is #1707 (that ticket is closed and covers an unrelated nested-generic-construction defect; an earlier draft of this work mischaracterized the blocker as #1707, corrected here after re-verifying end to end):
  - **#5443** — the `[project.packages]` array form for one package built from two alternative files (`_kernel/net/web_kernel.l` / `_kernel/jvm/web_kernel.l`, selected by feature) leaks the non-selected file's `extern type` binding when both files reuse the same local alias name (`ConcurrentDict[K, V]`) for genuinely different host types, producing a JVM class file with a dangling reference to the **.NET** type name. Fails all 5 `Web.RateLimitTests` cases with `Illegal class name "System/Collections/Concurrent/ConcurrentDictionary\`2/"`.
...
lyric-web/
  ...
  tests/
    ...
    jvm_server_smoke.l            real Undertow HTTP round trip (jvm; blocked on #5443/#5444, see Known gaps)
```
